...
- Refactor (Phase 1 of FloatingTurretEntity split): clone-mode logic extracted into new `CloneTurretController` (package-private). FloatingTurretEntity drops from 628 → 482 lines and no longer mixes player-mode and clone-mode private state in the same scope. Public API (`setupAsCloneTurret` / `getCloneOwner` / `hurt` / `isPickable`) signatures unchanged — all external callers (`PlayerCloneEntity.mirrorFrom`, `FloatingTurretRenderer`, `FloatingTurretProjectile`) compile and behave identically. Shared knobs (`ORBIT_RADIUS`/`ORBIT_HEIGHT`/`ORBIT_SPEED`/`PASSIVE_ATTACK_COOLDOWN`/`PASSIVE_RANGE`/`attackTimer`) demoted to package-private so the controller can read/write them. Player-mode extraction (Phase 2 → `PlayerTurretController`) deferred to a later pass once multiplayer behaviour is verified.
- 重構（FloatingTurretEntity 拆分 Phase 1）:clone 模式邏輯抽到新類別 `CloneTurretController`（package-private）。FloatingTurretEntity 從 628 行降到 482 行,玩家模式跟 clone 模式的私有狀態不再混在同一個 scope。Public API（`setupAsCloneTurret` / `getCloneOwner` / `hurt` / `isPickable`）簽名完全不變,所有外部 caller（`PlayerCloneEntity.mirrorFrom`、`FloatingTurretRenderer`、`FloatingTurretProjectile`）編譯與行為一致。共用常數（`ORBIT_RADIUS`/`ORBIT_HEIGHT`/`ORBIT_SPEED`/`PASSIVE_ATTACK_COOLDOWN`/`PASSIVE_RANGE`/`attackTimer`）降為 package-private 讓 controller 能讀寫。玩家模式抽出（Phase 2 → `PlayerTurretController`）等多人實機驗證後再做。

- New `FloatingTurretGameTests` (5 tests under `@GameTestHolder(koniava)`): `cloneTurret_immuneToPlayerMelee`, `cloneTurret_immuneToPlayerProjectile`, `cloneTurret_takesEnvironmentalDamage`, `cloneTurret_isPickableFalse`, `playerTurret_hurtFromMobStillWorks`. All five pass in `./gradlew runGameTestServer`. GameTest only covers logic; network-sync verification still requires manual `runServer` + dual-client testing.
- 新增 `FloatingTurretGameTests`(5 個 test 掛在 `@GameTestHolder(koniava)`):`cloneTurret_immuneToPlayerMelee`、`cloneTurret_immuneToPlayerProjectile`、`cloneTurret_takesEnvironmentalDamage`、`cloneTurret_isPickableFalse`、`playerTurret_hurtFromMobStillWorks`。`./gradlew runGameTestServer` 五個全 PASS。GameTest 只蓋邏輯層,network sync 還是要實機 `runServer` + 兩個 client 驗。
