...
You can now walk normally on a tilted spaceship. Standing on a slanted deck used to make you slide downhill and stutter; on a tilted ship, gravity now pulls you into the deck (along its surface) instead of straight down, so you stick to the deck and walk smoothly without sliding. You still stand upright and the camera does not flip (full deck-as-down reorientation is a later step). Riders also stay seated when a ship banks: rolling or pitching used to slowly slide you out of your chair, and the seat position now follows the ship's tilt.
現在可以在傾斜的飛船上正常走路了。以前站在斜甲板上會往低處滑、卡頓；傾斜的船上，重力現在會把你往甲板裡拉（沿甲板表面）而不是直直往下，所以你會黏在甲板上順順地走、不滑。人還是直立的、鏡頭不會翻（完整「甲板就是下」的翻轉是之後的步驟）。船側翻時乘客也會穩穩坐在椅子上：以前 roll/pitch 會慢慢把你滑出椅子，現在座位位置會跟著船的傾斜。
...
Artificial gravity phase 1: applyContraptionMovement redirects a supported player's downward gravity along the deck normal (orientation transform of (0,-1,0)) when the ship is tilted more than 1 degree, removing the world-down tangential component that caused sliding and the onGround flicker (rider stays upright, no camera/input reorientation yet). isSupporting was loosened to a 0.5-block foot probe (was 0.02) so a small float gap does not disable it. positionRider now folds SEAT_SIT_HEIGHT into the local seat position before rotatedWorldPoint, so the sit height follows roll/pitch and riders do not drift off the chair when banking. gametest tiltedDeckNoSlide: gravity-only on a 20-degree deck drifts 0.02 blocks (was 2.14).
人工重力 phase 1：applyContraptionMovement 在船傾斜超過 1 度且玩家被船支撐時，把向下重力重導向甲板法線（orientation 變換 (0,-1,0)），去掉造成滑動和 onGround 跳的世界下切向分量（人保持直立，還沒翻鏡頭/操作）。isSupporting 放寬成腳下 0.5 格探測（原 0.02），小浮點縫不會讓它失效。positionRider 把 SEAT_SIT_HEIGHT 併入 local 座位位置再 rotatedWorldPoint，座高跟著 roll/pitch、側翻時乘客不偏離椅子。gametest tiltedDeckNoSlide：20 度甲板只有重力時漂移 0.02 格（原 2.14）。
