...
Mining a block on a parked ship now removes exactly the block under your crosshair, even while walking: the break is taken from where your client is aiming instead of a separate server-side raycast that lagged behind your movement and sometimes hit a different block.
挖停著飛船上的方塊現在會精準挖掉準心對著的那一格，連走動時也是：挖的是你客戶端準心瞄準的格，不再是伺服器另外算、會落後你移動而有時挖到別格的射線。
...
Breaking a parked ship's block is now client-authoritative: ShipEntity.hurt is a no-op, a client AttackEntityEvent sends ShipBreakBlockPacket with getAimedLocalBlock, and the server runs the refactored public breakLocalBlock(player, local) on the received position (range-checked) instead of re-raycasting with the network-lagged server-side player position, so the broken block matches the client crosshair/outline.
挖停著飛船的方塊改成 client 權威：ShipEntity.hurt 變 no-op，client 的 AttackEntityEvent 用 getAimedLocalBlock 發 ShipBreakBlockPacket，server 對收到的座標（有範圍檢查）跑重構出來的公開 breakLocalBlock(player, local)，不再用有網路延遲的伺服器端玩家位置重新 raycast，所以挖掉的方塊跟 client 準心/外框一致。
