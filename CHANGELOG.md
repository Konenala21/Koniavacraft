...
- WIP (skill system recipes): the two skill-system pieces are now obtainable instead of creative-only. Added mana crafting table recipes for the Skill Encoder block (circuits + crystal + substrate, like the Mana Charger) and the Spell Core item (blank core + amethyst + crystal + refined dust, sitting alongside the other plug-in cores). Datagen only: run runData to emit the JSON. Not yet research-gated.
- WIP（技能系統配方）：技能系統的兩個關鍵物現在做得出來，不再是創造限定。新增魔力工作台配方：技能編碼台方塊（電路+水晶+基板，比照魔力充能台）、法術核心物品（空白核心+紫水晶+魔力水晶+精煉粉，跟其他插件核心並列）。純 datagen：要跑 runData 才生 JSON。尚未綁研究 gate。
