...
- Reworked the Mana Alloy Chestplate shield into installable plugins (two mutually exclusive types, Mk0-3, chestplate only). Type 1 Damage Reduction Shield: free passive damage reduction 40%/50%/60%/60%, and Mk3 also heals 50% of the damage you actually take back over 3 seconds. Type 2 Absorption Shield: blocks 100% of damage from a shield pool of 100/150/200/200; when the pool empties it auto-converts the chestplate's stored mana into shield (75 mana = 1 point, up to the cap and your available mana), and Mk3 additionally blocks a flat 20 damage per hit while the shield has energy. Only one shield type can be installed, and only one of it. The old always-on 40% passive shield is removed (no shield without a plugin now). All craftable at the Mana Crafting Table. Requires runData.
- 把魔力合金胸甲的護盾改成可安裝的插件（兩種互斥、各 Mk0-3、胸甲專屬）。類型一減傷護盾：被動免費減免 40%/50%/60%/60% 傷害，Mk3 額外把你「實際受到」的傷害的 50% 在 3 秒內回血。類型二吸收護盾：用 100/150/200/200 的護盾值 100% 擋傷；護盾打空時自動把胸甲儲存的魔力換成護盾（75 魔力 = 1 點，受上限與現有魔力限制），Mk3 還會在盾有值時每次受擊額外免傷 20 點。只能裝一種護盾、且只能裝一個。移除舊的常駐 40% 被動護盾（現在沒裝插件就沒有護盾）。皆可在魔力合成台製作。需跑 runData。
