...
Standing and walking on a spaceship is now smooth: no falling through the deck, no juddering or "sinking in" while you walk, and no resistance, on parked or mildly tilted ships. Blocks anywhere on a large ship (even the far end) are clickable again. Riders stay seated when the ship banks, and the first-person view leans with the ship instead of drifting off the seat. Steeply tilted decks still let you slide downhill (world gravity on a slope); walking on a steep or upside-down deck as if it were the floor is a future "deck is down" feature.
站在飛船上、走路現在順了：不會穿過甲板、走路不會抖或「陷下去」、也沒有阻力（停著或微斜的船）。大船上任何地方（連最尾端）的方塊都能點了。船側翻時乘客穩穩坐著，第一人稱視角會跟著船傾、不會飄離座位。大幅傾斜的甲板還是會讓你往低處滑（斜坡上的世界重力）；要在陡峭或顛倒的甲板上像踩地板一樣走，是之後的「甲板就是下」功能。
