...
Five new skill carriers (ways to deliver a skill), bringing the total to 13. Nova (兌) bursts around you, hitting and lightly knocking back everything nearby, great for a melee/self-reaction build. Shockwave (坤) is a wide expanding ring with lighter damage but a strong outward shove, for crowd control. Earthwave (艮) sends a wave forward along the ground in a cone and pops grounded foes up. Lob (坎) throws an arcing grenade that falls under gravity and triggers on impact, good for hitting over cover. Meteor (離) drops a strike from the sky onto where you are aiming. Put the matching aspect in the carrier slot in the encoder to use them.
新增五種技能載體（把技能送出去的方式），總數來到 13 種。環爆（兌）以你為中心爆發，打到並輕微擊退周圍所有敵人，適合近戰／自我反應流。衝擊波環（坤）是大範圍擴散環，傷害較低但有強力外推，適合控場。震波（艮）沿地面往前扇形推出一道波並把地面上的敵人掀起。拋物榴彈（坎）丟出受重力的拋物線手雷，落地觸發，適合越過掩體打。天降（離）對你瞄準的位置從天空砸下一擊。在編碼台把對應本源放進載體槽就能用。
...
- Fix: the Research Table screen stuttered every time it opened. It was the only custom screen still using the vanilla screen background, which runs the 1.21 blur post-effect on open (a per-open hitch). Overrode renderBackground to a cheap dim fill (0x55000000), matching the mod's other custom screens (turret/wand/armor upgrade, etc.). Opening sends a small AspectSyncPacket each time (~80 entries), which is fine and not the cause.
- 修復:研究台介面每次開都卡一下。它是唯一還在用 vanilla 介面背景的自訂 screen,而 vanilla 在開介面時會跑 1.21 的 blur post-effect(每次開的 hitch)。覆寫 renderBackground 成便宜的暗色填滿(0x55000000),跟模組其他自訂介面(浮游砲/法杖/盔甲升級等)一致。開啟時會送一個小的 AspectSyncPacket(約 80 筆),那個沒問題、不是元兇。
...
- Wired the Shockwave carrier to a shader visual (step 3 of the carrier-FX refactor). New CarrierFxType enum (SHOCKWAVE) + CarrierFxPacket (S2C, sent to players within 64 blocks) + client-side CarrierFxClient dispatcher. CarrierFx.shockwave now also sends the packet; the client plays it through the existing ManaStrikeShaderRenderer (its built-in expanding ring reads as the shockwave). gameplay still calls CarrierFx in one line; adding a shader carrier = one CarrierFxType entry + one CarrierFxClient case. Packet registered in ModNetworking. The handler is client-only so CarrierFxClient never loads on a dedicated server (same pattern as SkillCooldownPacket). Visual is reused as-is for now and can be tuned live.
- 把衝擊波載體接上 shader 視覺(載體 FX 重構第 3 步)。新增 CarrierFxType enum(SHOCKWAVE)+ CarrierFxPacket(S2C,送給 64 格內玩家)+ client 端 CarrierFxClient 派發器。CarrierFx.shockwave 現在多送這個 packet;client 用現成的 ManaStrikeShaderRenderer 播(它內建的擴散環就是衝擊波視覺)。gameplay 仍只呼叫 CarrierFx 一行;加 shader 載體 = 一個 CarrierFxType + 一個 CarrierFxClient case。packet 註冊在 ModNetworking。handler 只在 client 跑,所以 CarrierFxClient 不會在 dedicated server 載入(跟 SkillCooldownPacket 同 pattern)。視覺先直接重用,之後可實機微調。 Moved the inline particle code (slash sweep, pipeline beam, gravity field swirl, chain spark) out of SkillCompiler into a new server-side CarrierFx layer; SkillCompiler now calls CarrierFx.slash/beam/field/chainSpark. Pure structural move, behavior unchanged. Sets up adding shader-backed carriers (shockwave/orbital) via a CarrierFxPacket without touching hit/effect logic.
