...
- WIP (semantic scanning A+B): the word-vector matcher now exposes a candidate pool (top-N plausible aspects by cosine) and the world seed deals from it (AspectExpression.seededPick re-ranks the pool by genome seed). So item->aspect stays semantically sensible (accuracy) but which plausible aspect an item carries varies per world (replayability), and an item always resolves to something. Wired into BlockAspectResolver + RecipeInferenceEngine in place of the deterministic top-1-2. Global completeness "dealing" (guaranteeing every aspect appears somewhere in a world) is a separate later piece. Unit-tested (pick stays within the pool, deterministic per seed).
- WIP（語意掃描 A+B）：詞向量比對器現在給出候選池（cosine 前 N 個合理本源），由世界種子發牌（AspectExpression.seededPick 用 genome seed 重排候選池）。所以物品→本源仍語意合理（準確），但每個世界選到的合理本源不同（重玩性），且物品一定解析得出東西。接進 BlockAspectResolver + RecipeInferenceEngine 取代原本確定性 top-1-2。全域完整性「發牌」（保證每個本源在每世界都有來源）是之後另一塊。已單元測試（挑選不跑出池外、同種子可重現）。
