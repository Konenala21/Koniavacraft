...
You can now fly your spaceship to the Moon. While out in space, fly your ship into the Moon's gravity zone (the Moon orbits Earth) and it lands you in the Moon dimension above the surface, where you descend in low gravity and dig down through the regolith to the glowing lunar core. Fly back up high off the Moon and the ship returns you to space at the Moon's current orbital position. No extra requirement beyond being able to reach space.
現在可以把飛船飛到月球了。在太空時,把船飛進月球的引力區(月球繞地球轉),就會把你降落到月球維度的地表上空,你在低重力中往下降、挖穿月壤到發光的月核。在月球往上飛得夠高,船會把你帶回太空的月球當下軌道位置。除了能到太空之外不需要額外條件。

Five spaceship blocks got new hand-drawn 3D models and textures: the ship core, the assembly base, the assembly gantry, the mana fuel tank, and the ship seat. The fuel tank is now a glass-windowed tower whose mana shows as a cyan liquid level rising and falling with how much fuel it holds, so you can read a tank at a glance.
五個飛船方塊換上手繪的 3D 模型和貼圖:飛船核心、組裝底座、組裝支架、魔力燃料槽、飛船座椅。燃料槽現在是有玻璃窗的塔狀槽,魔力顯示成青色液面,會隨儲量升降,一眼就看得出哪槽滿哪槽空。
...
Fixed a crash when flying a spaceship fast, both hugging the ground in the normal world and out in space. The ship's terrain-collision check was force-loading the chunks along its path, and at high speed that meant generating/loading a flood of chunks every tick (in space they are all empty/ungenerated; near the ground they just cannot load fast enough), which could crash the game and, in space, let the ship outrun loading and disappear. The ship now only collides against chunks that are already loaded (it passes through not-yet-loaded terrain until it loads, same as before for ground-hugging) and skips terrain collision entirely in space, so fast and warp flight is stable.
修了高速駕駛飛船的崩潰,主世界貼地飛和太空都會。飛船的地形碰撞檢查會強制載入路徑上的區塊,高速時等於每 tick 生成/載入海量區塊(太空那些全是空的/未生成;貼地則是來不及載),會讓遊戲崩潰,在太空還會讓船跑贏載入而消失。飛船現在只跟「已載入的區塊」碰撞(沒載入的地形先穿過去等它載進來,貼地飛跟以前一樣),太空則完全跳過地形碰撞,所以高速和曲速飛行穩定了。
...
The first time planets render (e.g. Earth while flying up to space) no longer freezes the game. The large planet textures (Earth is 8K) are decoded on a background thread, and their GPU upload is now spread over many frames a few rows at a time instead of one big transfer, so the climb stays smooth. Earth fills in over about half a second the first time you see it, then stays cached. Full texture quality is kept (nothing is downscaled).
第一次渲染行星(例如飛上太空時的地球)不再卡住遊戲了。大張的行星貼圖(地球是 8K)在背景執行緒解碼,GPU 上傳現在也改成分很多幀、每次幾條 row,不再一次大傳輸,所以爬升全程順暢。第一次看到地球時會在約半秒內慢慢填滿,之後就快取住。畫質完整保留(沒有降採樣)。
...
The space transition no longer shows the vanilla "loading world" screen: when your ship crosses into or out of space, that brief loading screen is suppressed so the space sky stays on screen the whole time. You may see a frame or two of empty space while the new area loads, but no loading flash.
飛船進出太空時不再出現 vanilla 的「載入世界」畫面了:跨進/跨出太空那一刻,那張載入畫面會被攔掉,太空天空全程留在畫面上。新區域載入時你可能會看到一兩幀的空無一物,但不會有載入閃屏。
...
Placing and breaking blocks on a parked spaceship feels instant now. The ship used to wait for a server round trip before the block appeared or disappeared, so it lagged behind your click; placed blocks now show immediately, and broken blocks clear without the round-trip delay.
在停著的飛船上放/破壞方塊現在即時跟手了。以前要等 server 來回一趟方塊才出現或消失,比你的點擊慢半拍;現在放下去的方塊立刻顯示,破壞也不再有來回的延遲。

Double chests on a spaceship now look like double chests. Placing two chests side by side connects them into one large chest (54 slots), and they now also render as a connected double chest instead of two separate single chests. Placement is more forgiving too: the second chest aligns to the first, so they connect even if you were not looking in the exact same direction.
飛船上的大箱子現在看起來就是大箱子了。並排放兩個箱子會連成一個大箱子(54 格),而且現在畫面也會顯示成連起來的大箱子,不再是兩個分開的單箱。放置也更寬容:第二個箱子會對齊第一個,就算你視角不完全一致也連得起來。
...
Blocks built into a spaceship now actually work. Chests placed side by side combine into a double chest, machines and other special-render blocks (and item pedestals) now draw on the ship instead of being invisible, an item pedestal shows the item placed on it, a machine's input/output side configuration saves, dropped items rest on the deck and can be picked up again, and the structure-build wand (and its wand-core version) builds an altar's pillars and pedestals directly onto a ship-mounted altar.
組進飛船的方塊現在真的能用了。並排放的箱子會合併成大箱子，機器和其他有特殊渲染的方塊（還有物品底座）現在會在船上畫出來、不再隱形，物品底座會顯示上面擺的物品，機器的輸入/輸出方向設定存得進去，掉落物會停在甲板上、能撿回來，結構建造杖（和它的魔杖核心版）也能直接在船上的祭壇補柱子和底座。

Walking on a spaceship deck now feels like normal ground: the wading-through-water drag is gone and you stick to the floor instead of sinking in. A just-assembled ship parked on the ground is solid the instant it appears (no walking straight through it for the first moment), and collision on a steeply tilted ship is far more reliable: its walls, corners and bow stop you instead of letting you clip through, and the selection outline lines up with the block you are actually aiming at.
走在飛船甲板上現在像踩在正常地面：像在水裡的阻力沒了，也會黏在地板上不陷下去。剛組裝、停在地上的船一出現就是實心的（不會在第一瞬間直接穿過去），大幅傾斜的船碰撞也可靠多了：牆、角落、船頭都擋得住、不再穿過去，傾斜船上的選取外框也對齊你實際瞄準的方塊了。

Earth's oceans now catch a sun glint from space: the sunlight reflects off the sea as a bright moving highlight, while land stays matte.

Standing and walking on a spaceship is now smooth: no falling through the deck, no juddering or "sinking in", and no walking resistance, on parked or mildly tilted ships. You can no longer walk through the hull walls or ceiling (from inside or outside), and blocks anywhere on a large ship, including the interior, are clickable again. Riders stay seated when the ship banks, and the first-person view leans with the ship instead of drifting off the seat. Steeply tilted decks still let you slide downhill (world gravity on a slope); walking on a steep or upside-down deck as if it were the floor is a future "deck is down" feature.

Ships can now be assembled with diagonally-joined blocks: the assembly connects blocks that touch on an edge or a corner, not only face-to-face, so a hull that steps diagonally no longer leaves pieces behind.
站在飛船上、走路現在順了：不會穿過甲板、走路不會抖或「陷下去」、也沒有阻力（停著或微斜的船）。也不會再從船的牆壁或天花板穿過去（不管從裡面還外面），大船上任何地方（包含船艙內部）的方塊都能點了。船側翻時乘客穩穩坐著，第一人稱視角會跟著船傾、不會飄離座位。大幅傾斜的甲板還是會讓你往低處滑（斜坡上的世界重力）；要在陡峭或顛倒的甲板上像踩地板一樣走，是之後的「甲板就是下」功能。

飛船現在可以用斜對角接的方塊組裝：組裝會連「共用一條邊或一個角」的方塊，不再只認面對面，所以斜著疊上去的船體不會漏收方塊。

Big spaceships are now solid the whole way across. Previously, once you walked far enough from a large ship's centre you could suddenly drop straight through the deck as if the blocks were not there: the game simply was not finding the ship to collide against from out there. It now finds the ship from anywhere on it, so the deck is solid everywhere and the safety net works everywhere too.
大飛船現在整艘都是實心的了。以前你走離大船中心夠遠時，會突然直接穿過甲板、像方塊不存在：那是遊戲在那個位置根本沒「找到」船來碰撞。現在不管你在船上哪裡都找得到船，所以甲板到處都是實心的、安全網也到處都生效。

Standing and walking on a spaceship is sturdier. A separate safety net now catches you whenever you would clip below the deck and lifts you back onto the surface, and the old push-out that made movement feel resistant has been removed.
在飛船上站、走更穩了。多了一層安全網：萬一你穿到甲板下面，會被拉回表面；舊的那個讓移動有阻力感的推出機制也拿掉了。

Walking on a spaceship deck made of stairs, slabs or other non-full blocks no longer makes you slowly drift and fall through after a while. Level ships use standard block collision for this (stairs and slabs behave cleanly); tilted ships keep the rotated-collision path.
在用樓梯、半磚等非完整方塊鋪的飛船甲板上走，不再走一走就慢慢漂移、過一陣子掉下去。水平的船用標準方塊碰撞處理（樓梯、半磚都正常）；傾斜的船維持旋轉碰撞。

Falling onto a spaceship from a height no longer drops you straight through the deck. Fast falls (near terminal velocity) used to tunnel through even solid floors at certain spots; ship collision now keeps up with fast movement so you land on the deck instead.
從高處摔到飛船上不再直接穿過甲板。以前快速墜落（接近終端速度）會在某些點穿過去、連實心地板都擋不住；現在飛船碰撞跟得上快速移動，你會正常落在甲板上。

Spaceship collision no longer rubber-bands. Standing or walking on a parked ship used to glitch because the server kept shoving you off the deck while your own screen had you standing on it; the server now leaves player-versus-ship collision to your own client, so it stays smooth and consistent (this was likely behind a lot of the "I clip through it" feeling).
飛船碰撞不再 rubber-band（彈來彈去）。以前站/走在停著的船上會鬼畫面，因為伺服器一直把你從甲板上推開、但你自己畫面上站得好好的；現在伺服器把玩家對船的碰撞交給你自己的客戶端，所以變得順又一致（你之前很多「會穿過去」的感覺多半是這個造成的）。

Spaceships now fly in full 3D. Look up or down to pitch the nose, and A/D banks (rolls) the ship, so you can fly nose-up, nose-down or rolled instead of only level. The camera and your character tilt with the ship as it banks, and the third-person camera pulls back farther for bigger ships so the whole hull stays in view.
飛船現在能完整 3D 飛行。抬頭/低頭讓船頭俯仰，A/D 讓船側翻（roll），所以你能機頭朝上、朝下或翻滾，不再只能水平飛。船側翻時鏡頭和你的角色會跟著傾斜，第三人稱鏡頭也會依船大小自動拉遠、框得住整艘。

Ships can now be much larger (up to 2048 blocks), and big ships are far smoother to build with: the freeze when assembling a ship or placing a block is roughly halved, and a freshly placed block now shows up immediately instead of being briefly invisible.
船可以做得更大（上限 2048 方塊），而且大船蓋起來順很多：組裝飛船或放方塊的凍結大約砍一半，剛放下去的方塊也會立刻顯示、不再短暫透明。

Several spaceship collision problems are fixed. You can now board and stand on a steeply tilted ship without falling through, falling onto a parked ship from above no longer drops you straight through the deck (especially at the stern and edges), a ship stuck embedded in terrain can now move free instead of freezing in place, and machines built into a ship are solid so you no longer fall through them.
修了幾個飛船碰撞問題。現在可以登上、站在大幅傾斜的船上而不會穿過去；從上方摔到停著的船上不再直接穿過甲板（尤其船尾和邊角）；嵌進地形卡住的船能脫離、不再原地凍住；組進船的機器也是實心的，不會再穿過去掉下去。

Earth in space now uses 8K day, cloud and night textures so it stays sharp up close with realistic clouds, and a terrain normal map gives mountains and coastlines real relief shading from the sun angle.
太空中的地球現在用 8K 的白天、雲層和夜景貼圖，靠近看也清晰、雲層真實；加上地形法線圖，山脈和海岸線會依太陽角度有立體起伏陰影。

The full mana alloy armor set (helmet, chestplate, leggings, boots) now wears a custom model made by our partner Bksesame, replacing the plain vanilla armor look. Each piece shows its own part of the set, so you can mix and match and still see the right pieces on your body.
整套魔力合金盔甲（頭盔、胸甲、護腿、靴子）現在會穿上夥伴蕎麥麵製作的自訂模型，取代原本的 vanilla 盔甲外觀。每一件只顯示它自己那部分，所以混搭穿戴時身上也會正確顯示對應的件。

Machines and farms now actually work on a spaceship, even while it flies. Build mana machines, furnaces, crops and chests into your ship: they run, process, and grow, and right-clicking a machine or chest opens its real working interface. Behind the scenes the ship's blocks live in a hidden world where they tick normally; you never get teleported, and what you see on the ship mirrors what is really happening. Ships far from any player stop ticking their machines to save performance and resume when you return.
機器和農場現在真的能在飛船上運轉了，連飛行時也是。把魔力機器、熔爐、作物、箱子組進你的船：它們會運轉、加工、生長，右鍵機器或箱子會打開它真正在運作的介面。幕後飛船的方塊住在一個隱藏世界裡正常 tick；你不會被傳送，你在船上看到的就是真正在發生的。離所有玩家很遠的飛船會暫停機器運轉來省效能，你回來時自動恢復。
...
Blocks placed on a parked spaceship now orient like they would in the world (stairs, logs, furnaces and other directional blocks face the right way relative to how you are looking), and multi-block blocks place fully: doors, beds, and tall plants all place both of their parts. Auto-connecting blocks like fences may not link to ship neighbours yet.
放在停著的飛船上的方塊現在會像在世界裡一樣依朝向擺放（樓梯、原木、熔爐等有方向的方塊會依你看的方向朝對的方位），多方塊也會完整放置：門、床、高植物都會放好兩個部分。柵欄這種自動連接的方塊目前可能還不會跟船上的鄰塊連接。

You can now edit a parked spaceship in place without disassembling it. While the ship is not being piloted, left-click a block on it to break it (it drops, with any container contents) and right-click with a block in hand against a face to add a block. The core block cannot be broken (it is the ship's anchor). Interactive blocks (seats, doors, chests) still take priority on right-click, so aim at a plain face to place.
現在可以直接在原地編輯停著的飛船，不用拆解。只要沒有人在駕駛，左鍵船上的方塊就能挖掉它（會掉落，連容器內容一起），手持方塊右鍵對著某個面就能加方塊。核心方塊不能挖（它是飛船的錨點）。互動方塊（座椅、門、箱子）右鍵時仍優先，所以要對著普通的面才會放方塊。
...
Spaceship boarding is now per seat. Right-clicking the ship only does something when you aim at a seat: you board the exact chair you clicked (not just the next free one), and clicking the ship body does nothing. Only the two seats nearest the core are pilot seats (either can fly, and two people can fly together); chairs further out are passenger seats that cannot steer. Getting off now drops you next to your seat instead of on the core, and picks a spot that is not inside the ship so you no longer clip through it. The chair's hitbox matches its model now instead of being a full block, and sitting places you in the chair properly.
飛船上船改成依座位。右鍵飛船只有指到座椅才有反應：你會坐上**點到的那張椅子**（不再是下一個空位），點船身則沒反應。只有離核心最近的兩張椅子是駕駛位（都能開，兩人可一起駕駛）；更外面的椅子是乘客位、不能操控。下船改成出現在座位旁而非核心上，且會挑一個不在船體裡的位置，不再穿過船。椅子的碰撞箱現在貼合模型不再是整塊方塊，坐下也會正確坐進椅子。

You can now stand and walk on a spaceship's deck. Players and mobs collide with the ship's actual blocks (floor, walls, and recesses), and when the ship flies you are carried along with it, turning with the deck as it rotates. Sitting in a ship seat now places you properly in the chair facing forward instead of floating above it and facing the wrong way. A ship with no seat can no longer be piloted: add a seat (an autopilot module is planned for later).
現在可以站上飛船甲板走動了。玩家和生物會跟飛船的實際方塊碰撞（地板、牆壁、凹槽），飛船飛行時你會被一起帶著走、隨甲板轉向。坐進飛船座椅現在會正確坐進椅子裡、面向前方，不再浮在椅子上方或面向錯邊。沒有座椅的飛船無法駕駛：放一張座椅（無人駕駛模塊之後再做）。
...
Moon landing in ShipTravel (reuses the Earth gravity-zone pattern, no research/recipe gate). currentEarthSpacePos was generalized to planetSpacePos(space, id) which resolves the parent chain (the Moon orbits Earth: parent "earth", orbit 700). tick() space branch now checks the Moon's current orbital position first (Moon and Earth are ~700 apart so their gravity zones do not overlap): within MOON_GRAVITY_RADIUS it transitions to the MOON dimension arriving at (0.5, MOON_ARRIVE_Y=160, 0.5) so the ship descends; otherwise the Earth zone returns to the overworld as before. A new MOON branch returns to space when the ship climbs to MOON_EXIT_Y=256, arriving at the Moon's orbital position pushed outward (away from Earth). No Moon launch point is stored. Parked caveat: a 2-tall VoxelShape on the fuel tank (to match its 2-block model) crashed ShipEntity.isSupporting (the ship deck-carry collision math assumes block shapes stay within [0,1]; an oversized shape indexes the VoxelShape coord list out of bounds), so 2-block collision is not viable for a ship block without reworking the ship collision; reverted.
月球登陸,加在 ShipTravel(沿用地球引力區那套,不加研究/配方 gate)。currentEarthSpacePos 一般化成 planetSpacePos(space, id),解析母星鏈(月球繞地球:parent "earth"、軌道 700)。tick() 太空分支先檢查月球當下軌道位置(月球和地球差 ~700,引力區不重疊):在 MOON_GRAVITY_RADIUS 內就傳進 MOON 維度、抵達 (0.5, 160, 0.5) 讓船往下降;否則維持原本地球區回主世界。新增 MOON 分支:船爬到 MOON_EXIT_Y=256 就回太空,抵達月球軌道位置往外推(遠離地球)。月球不存發射點。Park 的坑:燃料槽為對齊 2 格模型給 2 格高 VoxelShape,崩 ShipEntity.isSupporting(船甲板 carry 碰撞假設形狀在 [0,1] 內,超大形狀索引爆掉),所以船方塊不大改碰撞就不能 2 格碰撞,已撤回。

Replaced the datagen models for 5 ship blocks (ship_core, ship_assembly_base, ship_assembly_gantry, mana_fuel_tank, ship_seat) with hand-written Blockbench models authored by 蕎麥麵. The .bbmodel files (model_format java_block, 64x64) were converted to MC block model JSON in src/main/resources/assets/koniava/models/block/ (a small Python script mapped elements/from/to/faces/uv/rotation, applied inflate, normalized from>to mirror cubes, and carried display transforms over). ModBlockStateProvider now points these blocks at the hand-written models via UncheckedModelFile (new customShipModel helper; shipSeatModel rewritten to reference block/ship_seat with the existing horizontalBlock (toYRot+180)%360 convention, so ShipSeatBlock's VoxelShape still aligns). ship_assembly_pad has no new art and keeps cubeAll. mana_fuel_tank.json uses render_type minecraft:cutout because its glass faces have alpha 0 cut-outs to see the liquid through. ManaFuelTankRenderer's fluid-cavity constants were realigned to the new model's inner core (x/z 3.5~12.5, Y 4, span 24/16; the tank is a tower that grows up to ~2 blocks tall). Run runData so the generated blockstate/model JSON for these blocks regenerates.
把 5 個飛船方塊(ship_core、ship_assembly_base、ship_assembly_gantry、mana_fuel_tank、ship_seat)的 datagen 模型換成蕎麥麵畫的手寫 Blockbench 模型。.bbmodel(model_format java_block,64x64)用一支 Python 小腳本轉成 MC block model JSON 放 src/main/resources/assets/koniava/models/block/(對應 elements/from/to/faces/uv/rotation,套 inflate,正規化 from>to 的 mirror cube,帶過 display)。ModBlockStateProvider 改用 UncheckedModelFile 指向手寫模型(新 customShipModel 輔助;shipSeatModel 重寫成指向 block/ship_seat,沿用既有 horizontalBlock 的 (toYRot+180)%360 約定,ShipSeatBlock 的 VoxelShape 不用動)。ship_assembly_pad 沒有新素材,維持 cubeAll。mana_fuel_tank.json 用 render_type minecraft:cutout,因為玻璃面有 alpha 0 鏤空才能看到內部液體。ManaFuelTankRenderer 的流體內腔常數對齊新模型的內芯(x/z 3.5~12.5、Y 4、span 24/16;槽是塔狀往上長到約 2 格高)。要跑 runData 重新產這些方塊的 blockstate/model JSON。
...
Fix a native crash (EXCEPTION_ACCESS_VIOLATION in C2-compiled ShipEntity.blockedBy, reading 0xFFFF...FF on the server thread) when flying fast, both ground-hugging in the overworld and in space; also fixes ships disappearing in space. Root cause: Level.getBlockState synchronously force-loads/generates a missing chunk, and ShipEntity's terrain collision (resolveTerrain -> blockedBy -> boxHitsTerrain) is sub-stepped at high speed (warp 30/tick -> ~60 steps x every hull block x a full-AABB getBlockState scan), so it force-loaded a flood of chunks per tick along the flight path. In space those chunks are all ungenerated; ground-hugging they just can't load in time. Fixes: (1) boxHitsTerrain now calls level().hasChunkAt(bx, bz) and skips any column whose chunk is not already loaded (no force-load; you pass through until it loads, which the SPEED_CAP comment already anticipated for ground-hugging). (2) resolveTerrain early-returns the move unchanged in dimension == SPACE (no terrain there; also avoids iterating hull blocks each tick). (3) boxHitsTerrain bails on non-finite AABB coords and clamps the Y scan to [minBuildHeight, maxBuildHeight-1]. Collision GameTests run in loaded chunks so behaviour is unchanged; 117/117 pass.
修高速飛行的 native crash(C2 編譯的 ShipEntity.blockedBy 裡 EXCEPTION_ACCESS_VIOLATION,server thread 讀 0xFFFF...FF),主世界貼地飛和太空都會;也修太空飛船消失。根因:Level.getBlockState 在區塊沒載入時會同步強制載入/生成,而 ShipEntity 的地形碰撞(resolveTerrain -> blockedBy -> boxHitsTerrain)高速時子步進(warp 30/tick -> ~60 段 x 每個外殼方塊 x 一整個 AABB 的 getBlockState),等於每 tick 沿路徑強制載入海量區塊。太空那些全未生成;貼地則來不及載。修法:(1) boxHitsTerrain 現在呼叫 level().hasChunkAt(bx, bz),沒載入的 column 直接跳過(不 force-load;先穿過去等它載入,貼地這點 SPEED_CAP 註解早預期過)。(2) resolveTerrain 在 dimension == SPACE 時提早回傳原 move(太空沒地形,也省每 tick 掃外殼)。(3) boxHitsTerrain 對非有限 AABB 座標放棄、Y 掃描夾 [minBuildHeight, maxBuildHeight-1]。碰撞 GameTest 在已載入區塊跑,行為不變;117/117 過。
...
Planet textures: off-thread decode + tiled GPU upload, to kill the first-render freeze without downscaling. Earth's textures are 8192x4096 (~134 MB each as RGBA). SpacePlanetManager.getTexture used to read + STB-decode synchronously on the render thread (multi-second freeze); the decode now runs on a small daemon ExecutorService (worker reads bytes + stbi_load_from_memory, parks a Decoded record with the native pixel buffer in a ConcurrentHashMap). The other half was the single 134 MB glTexSubImage2D upload, also a stall: getTexture now allocs the texture empty (glTexImage2D null) and registers an Upload; processUploads() runs once per frame and uploads a few rows per texture within a ~4 MB byte budget (duplicating the pixel buffer and seeking to rowsDone*width*4), so a big texture fills over ~33 frames. The texId only enters textureCache when fully uploaded (the planet renders procedurally until then, no half-filled garbage rows). decoding gates duplicate submits; reload() frees any in-flight native pixels to avoid a logout-mid-load leak. Full quality kept; no resize.
行星貼圖:背景解碼 + 分幀 GPU 上傳,不降採樣就消掉首次渲染卡頓。地球貼圖是 8192x4096(RGBA 各約 134 MB)。SpacePlanetManager.getTexture 以前在 render thread 同步讀 + STB 解碼(卡數秒);解碼現在跑在小的 daemon ExecutorService(worker 讀 bytes + stbi_load_from_memory,把 Decoded record 連 native 像素放進 ConcurrentHashMap)。另一半是那次 134 MB 的 glTexSubImage2D 整張上傳,也會卡:getTexture 現在先配置空 texture(glTexImage2D null)+ 登記 Upload;processUploads() 每幀跑一次,在 ~4 MB byte 預算內對每張貼圖上傳幾條 row(duplicate 像素 buffer + seek 到 rowsDone*width*4),大貼圖約 33 幀填滿。texId 要全部上傳完才進 textureCache(填滿前行星先程序化,不會露出半填的亂色)。decoding 防重複提交;reload() 釋放還在傳的 native 像素,避免登出在載入中途洩漏。畫質完整,不縮圖。
...
Fix: neighbour-aware block appearance (ArcaneConduit connection arms) was not updating on ship edits. ShipEntity.writeToShadow places shadow blocks with Block.UPDATE_CLIENTS (no UPDATE_NEIGHBORS) on purpose, because neighbour-update cascades are what froze big ships on edit. The placed block's own onPlace/scheduleTick still runs, so a conduit connects itself, but an existing conduit next to a newly placed non-conduit (fuel tank / machine) never received a neighborChanged and so never grew an arm. writeToShadow now calls refreshConduitConnections(shadow, sp): a bounded refresh of the placed cell + its 6 neighbours only, recomputing ArcaneConduitBlock.updateConnections and writing back with UPDATE_CLIENTS (no cascade); tickServerMirror then mirrors the corrected shadow state back to the contraption and clients. Covers both place and break (both funnel through writeToShadow).
修復:鄰居感知外觀(ArcaneConduit 連接臂)在船上編輯時不更新。ShipEntity.writeToShadow 故意用 Block.UPDATE_CLIENTS(不帶 UPDATE_NEIGHBORS)放影子方塊,因為鄰居更新連鎖正是大船編輯凍結的元兇。放下的方塊自己的 onPlace/scheduleTick 照跑,所以導管會接自己,但既有導管旁邊放了新的非導管(燃料槽/機器)時收不到 neighborChanged,就不會長臂。writeToShadow 現在呼叫 refreshConduitConnections(shadow, sp):只 bounded 刷新放下那格 + 6 鄰居,重算 ArcaneConduitBlock.updateConnections 寫回(UPDATE_CLIENTS 不連鎖);tickServerMirror 再把修正後的影子 state 鏡射回 contraption 和 client。放置和破壞都涵蓋(兩者都走 writeToShadow)。

Refactor: split three cohesive subsystems out of ShipEntity (2220 -> 1991 lines), pure structural moves with no behaviour change. (1) ShipFuelSystem owns the fuel/engine/warp state (engine count, fuel-tank / warp-core / intake local lists) plus recompute, incremental onBlockChanged, read/drain fuel, and tier(); the speed/fuel/enthalpy constants moved here too. (2) ShipTravel owns the altitude-triggered overworld <-> space transition (tick, the orbiting-Earth gravity-zone math, changeDimension passenger handoff, and the LaunchX/Z NBT); its SPACE_ENTRY_Y is now the single source of truth that the client AtmosphereTransition.Y_FADE_END references. (3) ShipPicking owns the view-ray block pick (pick / pickBlock / pickFace). ShipEntity keeps thin public delegators (getShipTier, getEngineCount, clientPick, getAimedLocalBlock, etc.) so external callers and the Pick record are unchanged. getShadow() is now package-private so the helpers can read the shadow tanks. Compiles green and all 117 GameTests pass.
重構:從 ShipEntity 抽出三個內聚子系統(2220 -> 1991 行),純結構搬移無行為改變。(1) ShipFuelSystem 持有燃料/引擎/曲速 state(引擎數、燃料槽/曲速核心/進料口 local 清單)+ recompute、增量 onBlockChanged、讀/抽燃料、tier();速度/燃料/魔焓常數也搬過去。(2) ShipTravel 持有高度觸發的主世界 <-> 太空轉場(tick、公轉地球引力區數學、changeDimension 乘客交接、LaunchX/Z NBT);它的 SPACE_ENTRY_Y 現在是唯一真實來源,client 的 AtmosphereTransition.Y_FADE_END 引用它。(3) ShipPicking 持有視線方塊拾取(pick / pickBlock / pickFace)。ShipEntity 留薄公開 delegator(getShipTier、getEngineCount、clientPick、getAimedLocalBlock 等),外部呼叫者和 Pick record 不變。getShadow() 改 package-private 讓 helper 讀影子燃料槽。編譯綠,117 個 GameTest 全過。

Seamless overworld -> space transition: atmosphere fade + planet-shader Earth-below + background preload. The changeDimension swap is unavoidable (machines tick in the shadow dimension, planets live at absolute orbit coordinates), so seamlessness is achieved by making both sides look identical at the swap altitude rather than removing the swap. New client helper AtmosphereTransition.blend(mc) returns 0..1 = smoothstep(camY, Y_FADE_START=700, Y_FADE_END=1000), but only while riding a ShipEntity with getShipTier() >= 1 in the overworld (Y_FADE_END must equal ShipEntity.SPACE_ENTRY_Y). Three pieces: (1) SpaceSkyRenderer now also runs in the overworld when blend > 0, alpha-blending the star sky over the vanilla sky via a new uAtmosphere fsh uniform (= blend); space/moon still render opaque at atmo = 1. (2) ClientEffectEvents.onFogColor (ViewportEvent.ComputeFogColor) scales the fog colour toward black by blend, killing the residual horizon blue. (3) SpacePlanetManager.renderAscentEarth draws Earth straight below the camera using the existing atmosphere PlanetRenderer, receding (dist 150 -> 280) and fading in (alpha = blend) so at Y1000 it matches the ~15 deg apparent radius seen on arrival in space (gravity radius 280, Earth physicalRadius 76). Running the space shaders in the overworld during the climb also pre-warms their GL init/textures, so arriving in space no longer has the cold-start flicker. Client-only, no server change. Needs an in-game test.
無縫主世界 -> 太空轉場:大氣層漸變 + 行星 shader 畫腳下地球 + 背景預載。changeDimension 切換省不掉(機器在影子維度 tick、行星在絕對軌道座標),所以無縫是靠「讓切換高度時兩邊畫面一致」,不是消除切換。新 client helper AtmosphereTransition.blend(mc) 回 0..1 = smoothstep(camY, Y_FADE_START=700, Y_FADE_END=1000),但只在主世界騎著 getShipTier() >= 1 的 ShipEntity 時生效(Y_FADE_END 必須等於 ShipEntity.SPACE_ENTRY_Y)。三塊:(1) SpaceSkyRenderer 現在 blend > 0 時在主世界也跑,透過新的 uAtmosphere fsh uniform(= blend)把星空 alpha blend 疊在原版天空上;太空/月球仍 atmo = 1 不透明。(2) ClientEffectEvents.onFogColor(ViewportEvent.ComputeFogColor)把霧色隨 blend 染黑,消掉地平線殘留的藍。(3) SpacePlanetManager.renderAscentEarth 用現有大氣 PlanetRenderer 在相機正下方畫地球,後退(距離 150 -> 280)+ 淡入(alpha = blend),所以 Y1000 時對齊太空那邊抵達時看到的 ~15 度視角半徑(引力半徑 280、地球 physicalRadius 76)。爬升時在主世界跑太空 shader 也順便預熱了 GL init/貼圖,抵達太空不再有冷啟動閃爍。純 client、無 server 改動。需要實機測。

Altitude-triggered cross-dimension transition for spaceships (overworld <-> space), tier-gated. ShipEntity.tickDimensionTransition runs each server tick: in the overworld at Y >= SPACE_ENTRY_Y it checks getShipTier() >= ORBIT_MIN_TIER and, if met, moves the ship to the space dimension; in space at Y <= SPACE_EXIT_Y it returns to the overworld (no tier gate on the return). The ship is a contraption entity, so the move uses changeDimension(DimensionTransition) which recreates it in the target level - the save NBT carries the contraption + shadowAnchor, and the new ship's tick re-acquires the shadow force-load via ensureShadowForceLoaded, so machines keep running. The ship's remove() already preserves the shadow on a non-KILLED removal (dimension change), so the shadow blocks survive. Passengers are dismounted, then teleported to the destination and re-mounted on the recreated ship. This is the functional core; the seamless atmosphere fade + background preload come next, and planet gravity-zone landings later. Needs an in-game test (cross-dim contraption + passenger handoff).
飛船的高度觸發跨維度轉場(主世界 <-> 太空),由 tier gate。ShipEntity.tickDimensionTransition 每個 server tick 跑:主世界 Y >= SPACE_ENTRY_Y 時檢查 getShipTier() >= ORBIT_MIN_TIER,夠就把船搬進太空維度;太空 Y <= SPACE_EXIT_Y 就傳回主世界(回程不卡 tier)。船是 contraption entity,搬移用 changeDimension(DimensionTransition) 在目標維度重建:save NBT 帶著 contraption + shadowAnchor,新船 tick 透過 ensureShadowForceLoaded 重接影子 force-load,機器繼續跑。船的 remove() 在非 KILLED 移除(換維度)時本來就保留影子,所以影子方塊不會掉。乘客先卸載,再傳到目的地、重新騎上重建的船。這是功能核心;無縫大氣層漸變 + 背景預載接下來做,星球引力區登陸之後做。需要實機測(跨維度 contraption + 乘客交接)。

Spaceship fuel/tier foundation (魔焓 / mana-enthalpy), starting from mana. The fuel/tier progression was redesigned to start at mana, not chemical (the mod is mana-driven from early game and building a ship already needs mana, so there is no pre-mana phase). 5 fuel families across 20 tiers (basic mana 1-5, refined mana 6-9, aspect 10-14, cosmic aspect 15-19, ultimate 20), gated by engine generations (the Mana Engine burns up to 魔焓 9). This commit lays the foundation: ShipEntity.getShipTier() = min(loaded fuel 魔焓 the engine can burn, engine 魔焓 cap), computed from the synced DATA_FUEL + engineCount so it works client-side, and the ship HUD shows the tier. For now only basic mana (魔焓 5) + the Mana Engine (cap 9) exist, so a fuelled ship reads Tier 5. Higher families, engine generations, and destination gating come next. Design in the space-system doc / project_ship_fuel_tier_system memory.
飛船燃料/階級系統地基(魔焓 / mana-enthalpy),從魔力起跳。燃料/階級進度重新設計成從魔力開始,不是化學(模組從早期就魔力驅動、蓋船就需魔力,沒有 pre-mana 階段)。5 個燃料家族跨 20 階(基礎魔力 1-5、精煉魔力 6-9、本源 10-14、宇宙本源 15-19、終極 20),由引擎世代 gate(魔力引擎燒到魔焓 9)。這次先立地基:ShipEntity.getShipTier() = min(引擎燒得動的最高燃料魔焓, 引擎魔焓上限),用同步的 DATA_FUEL + engineCount 算所以 client 端也能算,船 HUD 顯示階級。目前只有基礎魔力(魔焓 5)+ 魔力引擎(上限 9),所以有燃料的船顯示 Tier 5。更高家族、引擎世代、目的地 gate 接下來做。設計在太空系統 doc / project_ship_fuel_tier_system 記憶。

Client-side prediction for placing/breaking blocks on a parked ship. Ship edits go client -> ShipInteractPacket/ShipBreakBlockPacket -> server -> ShipBlockUpdatePacket -> client, so the block only appeared/disappeared after a round trip (~1 tick / ~50ms in singleplayer), unlike vanilla's instant local prediction. The interactWithPick place branch now calls clientPredictPlace on the client (computePlacementState + updateContraptionBlock, which feeds pendingVisualBlocks for an instant draw), and ShipBlockBreakHandler calls clientPredictBreak before sending the packet. Both mirror the server accept conditions (core block, structural block, MAX_BLOCKS) to avoid predicting a placement the server would reject; the authoritative ShipBlockUpdatePacket reconciles ~50ms later (in singleplayer client and server run the same logic so the prediction matches). Placement is instant via pendingVisualBlocks; breaking removes the round trip but the block still clears on the next mesh re-bake.
停著的飛船放/破壞方塊改成 client 端預測。船的編輯走 client -> ShipInteractPacket/ShipBreakBlockPacket -> server -> ShipBlockUpdatePacket -> client,所以方塊要等封包來回(單機 ~1 tick / ~50ms)才出現/消失,不像 vanilla 本地即時。interactWithPick 放置分支現在 client 端會呼叫 clientPredictPlace(computePlacementState + updateContraptionBlock,後者餵 pendingVisualBlocks 即時畫),ShipBlockBreakHandler 送封包前呼叫 clientPredictBreak。兩者都鏡射 server 接受條件(核心方塊、結構方塊、MAX_BLOCKS)避免預測 server 會拒絕的放置;權威 ShipBlockUpdatePacket ~50ms 後對齊(單機 client/server 跑同邏輯,預測必中)。放置靠 pendingVisualBlocks 即時;破壞省掉來回但方塊仍在下次 mesh 重烤時才清掉。

Ship render BEs now update their cached blockState on same-type changes. updateContraptionBlock only recreated a render BlockEntity when the BE type changed or was missing; on a same-type blockstate change (e.g. a chest going SINGLE -> LEFT/RIGHT when it merges) it kept the existing BE but never updated that BE's cached blockState, so its BER drew the stale state - a merged double chest stayed functionally double (54-slot, via the shadow chest's menu) but rendered as two single chests. Now the keep-existing branch calls existing.setBlockState(state) (and re-registers it in the render world), so the BER draws the new state while the BE's transient state (chest lid openness) is preserved.
飛船 render BE 在「同型別 blockstate 變化」時現在會更新它快取的 blockState。updateContraptionBlock 只有在 BE 型別改變或不存在時才重建 render BlockEntity;同型別的 blockstate 變化(例如箱子合併時 SINGLE -> LEFT/RIGHT)會保留既有 BE 但從沒更新那 BE 快取的 blockState,所以它的 BER 畫的是舊狀態 — 合併好的大箱子功能上是雙箱(54 格,靠影子箱子的選單)卻畫成兩個單箱。現在保留分支會呼叫 existing.setBlockState(state)(並重新註冊到 render world),BER 就畫新狀態,同時 BE 的 transient 狀態(箱子蓋子開合度)保留。

Ship collision shape no longer uses Shapes.or (spark-confirmed the real culprit). A spark profile of the server thread showed localCollisionShape() at 32.89% of the entire thread with per-tick spikes to 501ms, all in Shapes.or -> Shapes.joinUnoptimized -> BitSetDiscreteVoxelShape.forAllBoxes, reached from both Entity collision (the collideWithShips mixin) and the ship's own applyContraptionMovement. Partial-collision blocks (stairs, slabs) were accumulated with Shapes.or and the result merged with the whole-ship bitset; that final merge iterates every box of the whole ship, so even a few partials made each rebuild O(ship). Replaced with a 2x-resolution bitset that samples each block's collision boxes (cs.toAabbs) into 2x2x2 sub-voxels: stairs/slabs keep a walkable 0.5 step (<= the 0.6 auto-step) but there is zero Shapes.or, so building the shape is a plain O(ship) bitset fill. This was the actual fix; the debounce and shadow-skylight changes below helped but were not the main cause. 116 gametests green.
飛船碰撞形狀不再用 Shapes.or(spark 證實的真兇)。server thread 的 spark 剖析顯示 localCollisionShape() 佔整條 thread 的 32.89%、單 tick 尖刺到 501ms,全在 Shapes.or -> Shapes.joinUnoptimized -> BitSetDiscreteVoxelShape.forAllBoxes,而且實體碰撞(collideWithShips mixin)跟船自己的 applyContraptionMovement 兩條路都會走到。partial-collision 方塊(樓梯、半磚)用 Shapes.or 累積後再跟整艘 bitset 合併,那次合併會掃整艘每個 box,所以只要有幾個 partial,每次重建就 O(ship)。改成 2× 解析度 bitset,把每方塊的碰撞盒(cs.toAabbs)取樣進 2×2×2 子體素:樓梯半磚保留可走的 0.5 台階(<= 自動上階 0.6),但零 Shapes.or,建構就是單純 O(ship) 填 bitset。這才是真正的修;下面的 debounce 跟影子天光有幫助但不是主因。116 gametest 綠。

Ship collision shape rebuild is now debounced (the real per-tick edit cost). localCollisionShape() builds an O(whole-ship) VoxelShape (BitSet fill over every block + Shapes.or for partial-collision blocks). It was invalidated on every edit and rebuilt on the next collision check - which happens every server tick while a player stands on the (parked, being-edited) ship - so continuous editing rebuilt the whole-ship collision shape every tick on the server thread. This was invisible to per-operation timing because updateContraptionBlock only nulled the cache (cheap); the rebuild was deferred to the collision check. Now an edit marks the shape dirty and it rebuilds once after a 200ms debounce; the cached shape (the deck you stand on) stays valid in the meantime, only a just-placed block's collision lags ~200ms.
飛船碰撞形狀重建改成 debounce(真正的每 tick 編輯成本)。localCollisionShape() 建一個 O(整艘) 的 VoxelShape(逐方塊 BitSet fill + 非滿格方塊的 Shapes.or)。它每次編輯都被作廢、下次碰撞檢查時重建,而玩家站在(停著、編輯中的)船上時每個 server tick 都會碰撞檢查,所以連續編輯就在 server thread 每 tick 重建整艘碰撞形狀。前面逐 op 計時量不到它,因為 updateContraptionBlock 只是把快取設 null(便宜),重建延遲到碰撞檢查。現在編輯只標記髒,停手 200ms 後才重建一次;期間用舊的快取形狀(你站的甲板照樣有碰撞),只有剛放的方塊碰撞晚 ~200ms 生效。
...
When an altar forms/upgrades shadow-side it converts corner mana_block to altar_pillar and rings to resonance_ring in place; neither is an EntityBlock, so they are only mirrored to the visible ship via structureMirror, and they were never registered there (the conversion happens at formation, not when the structure wand runs, so wiring it into syncNewShadowBlocksToContraption was the wrong place and never fired for the player's flow). tickServerMirror now, whenever it mirrors an aspect_altar core (itself an EntityBlock, always in dynamicMirrorBlocks), adds that altar's AltarGeometry pillar and ring offsets to structureMirror (invalidating dynamicMirrorCache when new), so the next mirror pass syncs the converted blocks. No structure-wand step needed.
祭壇在影子端成形/升級時，會就地把角落 mana_block 換成 altar_pillar、把環換成 resonance_ring；兩者都不是 EntityBlock，只能靠 structureMirror 鏡射到視覺船，而它們從沒被登記進去（轉換發生在「成形」，不是用結構杖時，所以接在 syncNewShadowBlocksToContraption 是錯的地方、玩家的流程根本沒觸發）。tickServerMirror 現在每次鏡射到 aspect_altar 核心（核心本身是 EntityBlock、必在 dynamicMirrorBlocks）時，就把該祭壇的 AltarGeometry 柱子與環偏移加進 structureMirror（有新增就失效 dynamicMirrorCache），下一輪鏡射就同步轉換後的方塊。不需要結構杖。
...
Ship-mounted block entities now render against a ported Create VirtualRenderWorld (space.ship.virtualworld: VirtualRenderWorld + VirtualChunk/Section/Source + VirtualLevelEntityGetter, MIT, package-only changes plus removing the Flywheel VisualizationLevel), a contraption-aware fake Level set on each render BE so BERs that query getBlockState/getBlockEntity/neighbours work (chest DoubleBlockCombiner, machine block checks). Live BE NBT (pedestal item, machine contents) reaches clients via a new ShipBlockEntityDataPacket, sent from tickServerMirror when the NBT changes; updateContraptionBlockEntityData reloads just that one render BE. Chest double-combine is reproduced against the contraption in placeState (computePlacementState uses a real-world BlockPlaceContext and cannot see contraption neighbours, so vanilla leaves chests SINGLE). ConfigDirectionUpdatePacket and the batch variant now resolve the target BE from the player's open UniversalConfigMenu (the shadow BE) instead of player.level()+8-block range, so machine I/O config saves on ships. syncNewShadowBlocksToContraption scans the shadow around an altar after a structure-wand interaction and pulls blocks the wand built there into the contraption, sending ShipBlockUpdatePacket (state) and ShipBlockEntityDataPacket (NBT) so clients render them. The standalone StructureBuildWandItem and WandCoreBehavior.STRUCTURE_BUILD now share a WandStructure / RequiredBlock / WandStructures abstraction and a WandStructureBuilder loop (manaPerBlock parameterises the free wand vs the mana-charging core); adding a new multiblock is one WandStructure class. Items use the same applyContraptionMovement collision as players so they rest on the deck instead of phasing through. Tilt twitch on parked tilted ships fixed by setting OBB step-up maxStep to 0.
飛船上的方塊實體現在對著移植自 Create 的 VirtualRenderWorld（space.ship.virtualworld：VirtualRenderWorld + VirtualChunk/Section/Source + VirtualLevelEntityGetter，MIT，只改 package 並移除 Flywheel 的 VisualizationLevel）渲染，這個 contraption 感知的假 Level 設給每個 render BE，所以會查 getBlockState/getBlockEntity/鄰居的 BER 正常了（箱子 DoubleBlockCombiner、機器方塊判定）。即時 BE NBT（底座物品、機器內容）由 tickServerMirror 在 NBT 變動時用新的 ShipBlockEntityDataPacket 傳給客戶端；updateContraptionBlockEntityData 只重建那一格 render BE。箱子 double 合併在 placeState 對 contraption 重現（computePlacementState 用真實世界的 BlockPlaceContext、看不到 contraption 鄰居，所以 vanilla 把箱子留成 SINGLE）。ConfigDirectionUpdatePacket 和 batch 版改從玩家開著的 UniversalConfigMenu（影子 BE）取目標 BE，不是 player.level()+8 格距離，所以機器 I/O 設定在船上存得進去。syncNewShadowBlocksToContraption 在結構杖互動後掃祭壇周圍的影子、把杖在那裡蓋的方塊收回 contraption，並發 ShipBlockUpdatePacket（state）和 ShipBlockEntityDataPacket（NBT）讓客戶端渲染。獨立的 StructureBuildWandItem 與 WandCoreBehavior.STRUCTURE_BUILD 現在共用 WandStructure / RequiredBlock / WandStructures 抽象和 WandStructureBuilder loop（manaPerBlock 區分免費杖 vs 扣魔力核心）；加新多方塊結構就是一個 WandStructure。掉落物用跟玩家同套 applyContraptionMovement 碰撞，所以會停在甲板上、不再穿過去。停著的傾斜船上的抽搐由 OBB step-up maxStep 設 0 修掉。

Ship deck collision reworked onto a ported Create OBB continuous collider. space.ship.collision (Matrix3d/OrientedBB/CollisionList/ContinuousOBBCollider) is ported from Create (MIT; only package + catnip Iterate->boolean[] changed); Matrix3d gains set(Quaternionfc). applyContraptionMovement routes every ship (not just tilted) through obbCollideLocal because vanilla collideBoundingBox also corner-cuts a convex corner and never resolves the resulting penetration. obbCollideLocal sub-steps the motion (N=4): per sub-step it does continuous collide-and-slide (toImpact via temporalResponse + slide off the normal + collisionResponse push-out) then a zero-motion collideMany MTV push-out loop; the OBB rotation is q0.conjugate, the colliders are the cached CollisionList of the collision shape (collisionListCache, invalidated with the shape). setOldPosAndRot is now called at assembly (after placeAtShipCenter) and at the end of readSpawnData so xOld/yRotO are correct on frame 0 (a fresh ship was colliding in a frame anchored at 0,0,0). makeBoundingBox returns a cube centred on the ship's 3D centre with side = the bounds diagonal so the ship is pickable/collidable at any attitude. ShipOutlineRenderer uses ship.orientation(pt) (was yaw-only). resolveOverlap tests a half-size centre box (only real penetration, not surface contact) so deck walking is not pushed every tick; ShipDeckCarryHandler sets onGround + clears fallDistance when isSupporting (Create does the same, since ship blocks are not in the world). Diagnostic gametests diagShipCollisionDirections / diagCornerMultiTick + stationaryShipBlocksHorizontalWalk / tiltedShipBlocksWalkOBB. Known gap: an upright ship's exact-diagonal convex corner (measure-zero) still penetrates ~1.0; the single-axis MTV push-out converges to a still-penetrating fixed point there. Perf: up to N*(1+3) collideMany per entity per tick, but the broad phase early-outs away from ships.
The OBB-vs-vanilla tilt threshold dropped from 10 to 2 degrees once artificial gravity made the OBB's vertical stop consistent: vanilla collideBoundingBox can collide a near-level floor but not tilted walls (world-aligned box vs the rotated wall in ship-local space), so tilted ships route walls through the OBB again. The seated-rider model tilt (RenderPlayerEvent) now pivots ~0.3 above the feet (the seat) instead of the body centre, so riders no longer swing out of the chair when the ship banks. Ocean specular: earth_specular.png on sampler unit 4 with uHasSpec, a Blinn-Phong glint (exp 64, warm-white) gated to the lit ocean; renderAtmosphere gained a specTexId param.
人工重力讓 OBB 的垂直擋住一致後，OBB-vs-vanilla 的傾斜門檻從 10 度降到 2 度：vanilla collideBoundingBox 能擋近水平地板但擋不了傾斜的牆（世界對齊盒 vs 船 local 框轉過的牆），所以傾斜船的牆改走 OBB。坐姿乘客模型傾斜（RenderPlayerEvent）支點改成腳上方 ~0.3（座位）而非身體中心，側翻時乘客不再甩出椅子。海洋 specular：earth_specular.png 綁 sampler unit 4 + uHasSpec，Blinn-Phong 反光（指數 64、暖白）限受光海洋；renderAtmosphere 加 specTexId 參數。

Ship-on-deck collision robustness pass. Ship lookup uses level.getEntitiesOfClass with inflate(128) (per-level-correct); an earlier shared-static registry was reverted because in single-player the client and server share the static set and the s.level()==level filter dropped the player's own ship (whole ship phased through). GameRendererShipPickMixin re-runs a ship raycast after GameRenderer.pick so far-deck blocks on a big ship are clickable. The snap safety net was removed (it raycast down and could lift an indoor rider through the ceiling); the per-tick collision already catches falls. An attempted "artificial gravity phase 1" (redirecting a supported player's gravity along the deck normal) was added then removed: a gametest (tiltedDeckWalkFeel) showed it made the foot bob 1.03<->1.13 each tick (the walking twitch/sinking), and standing zeroes downward velocity when grounded so a 5-degree deck slides only 0.03 blocks anyway. positionRider keeps drivers and passengers on the full ship orientation; ShipRiderViewHandler tilts the rider model about the feet and offsets the first-person eye along orientation(partialTick). tiltAngleSweep documents that collision holds at every angle but world gravity slides the rider downhill (5deg=0.03 .. 85deg=4.8 over 3s); standing on steep/inverted decks needs deck-is-down (phase 2/3). Follow-up fixes: pushOutPenetration adds a zero-motion MTV pass after collideBoundingBox on upright ships, because vanilla collideBoundingBox does not push an already-overlapping box out (so a player who entered the hull walked straight through; gametest box at x=5.3 reached 13.30, now 4.70). The pick mixin now uses ship.getAimedLocalBlock instead of clipping the cube (clip from inside the cube returns empty, so interior blocks were not clickable). The horizontal collision snap tolerance was widened to 5e-3: a ~1-degree tilt leaks ~0.003 of the deck's local-Y stop into world X/Z, above the old 1e-4 snap, so Entity.move zeroed horizontal velocity every tick (the walk resistance). Assembly flood-fill is now 26-connected (faces+edges+corners) so diagonally-joined blocks are collected.
tiltAngleSweep 紀錄碰撞任何角度都 hold... 後續修正：pushOutPenetration 在直立船的 collideBoundingBox 後加一個零移動 MTV pass，因為 vanilla collideBoundingBox 不會把已重疊的盒子推出（進了船體的玩家會直接走穿；gametest 盒子從 x=5.3 跑到 13.30，現在 4.70）。pick mixin 改用 ship.getAimedLocalBlock 而非 clip cube（站在 cube 內 clip 回傳空，內部方塊點不到）。水平碰撞 snap 容差放寬到 5e-3：約 1 度傾斜會讓甲板 local-Y 歸零洩漏 ~0.003 到世界 X/Z，超過舊的 1e-4，導致 Entity.move 每 tick 歸零水平速度（走路阻力）。組裝 flood-fill 改成 26 連通（面+邊+角），斜對角接的方塊也收。
飛船甲板碰撞穩定性整理。查飛船改用 level.getEntitiesOfClass + inflate(128)（per-level 正確）；先前的共用 static registry 已回退，因為單機 client/server 共用 static、`s.level()==level` 會把玩家自己的船濾掉（整台船穿）。GameRendererShipPickMixin 在 GameRenderer.pick 後補一次船射線，大船遠端的方塊才點得到。移除往下射線的防陷安全網（它會把室內乘客往上拉穿天花板）；每-tick 碰撞本來就接得住。曾加又移除「人工重力 phase 1」（把被支撐玩家的重力沿甲板法線重導向）：gametest tiltedDeckWalkFeel 顯示它讓腳底每 tick 在 1.03↔1.13 彈（走路抽搐/陷下去），而站著 grounded 會把下墜速度歸零、5 度甲板本來就只滑 0.03 格。positionRider 讓駕駛和乘客都跟完整船姿勢；ShipRiderViewHandler 把乘客模型繞腳底傾斜、第一人稱眼睛沿 orientation(partialTick) 偏移。tiltAngleSweep 紀錄碰撞任何角度都 hold 但世界重力讓人沿坡滑（5度=0.03 .. 85度=4.8／3秒）；陡峭/顛倒甲板要站需 deck-is-down（phase 2/3）。

Both EntityShipCollisionMixin and ShipDeckCarryHandler now inflate their getEntitiesOfClass(ShipEntity) broad-phase search to 48 (was 1.0 / 0.5). Entities are bucketed in the section of their position, so a large ship registered only in its centre section is invisible to a search box that has moved into a different section, even though the ship's bounding box still covers the searcher (caught by logging: shipsInSearch=0 with the player provably inside the ship bb). This was the root of "I fall straight through, like there is no block" on big ships: with no ship found, neither the collision nor the safety net ran. The wider search is filtered back down by applyContraptionMovement's gate and a real-bb narrow check.
EntityShipCollisionMixin 和 ShipDeckCarryHandler 的 getEntitiesOfClass(ShipEntity) 廣相位搜尋盒放大到 48（原 1.0 / 0.5）。實體用「位置所在 section」分桶，大船只登記在中心 section，搜尋盒一旦移到別的 section 就找不到（雖然船 bb 還蓋著搜尋者；靠 log 抓到：shipsInSearch=0 但玩家明明在船 bb 內）。這就是大船「像沒方塊直接穿」的根因：找不到船，碰撞和安全網都不跑。放大的搜尋由 applyContraptionMovement 的 gate + 真 bb 收窄過濾回來。

Added ShipEntity.snapToDeckSurface, an anti-clip safety net ported from Create's savePlayerFromClipping: four world-down ray-casts from the rider's foot corners, transformed into the ship-local frame, against the collision shape; if the nearest hit is above the feet the rider has clipped through, so snap the feet to it and zero downward velocity. ShipDeckCarryHandler now calls this instead of resolveOverlap (the box-overlap push was what produced the "movement has resistance" feel; a ray-cast snap does not push horizontally). The insight from reading Create: its robustness comes from this safety net, not from a perfect per-tick collision pass.
新增 ShipEntity.snapToDeckSurface，移植自 Create savePlayerFromClipping 的防穿安全網：從乘客腳四角往世界下射 4 條線、轉進船 local 框打碰撞形狀；最近命中在腳上方=穿下去了，把腳拉回該表面並歸零下墜速度。ShipDeckCarryHandler 改呼叫這個取代 resolveOverlap（盒重疊推就是「移動有阻力」的來源；raycast 拉回不會水平推）。讀 Create 的領悟：它的 robust 是靠這層安全網，不是完美的每-tick 碰撞。

applyContraptionMovement routes upright (yaw-only) ships back through vanilla Entity.collideBoundingBox and reserves the ported OBB collider for pitched/rolled ships only. Walk-sim gametests (solidDeckWalkNoFall vs stairsDeckWalkNoFall) showed the OBB collide-and-slide + push-out accumulates a per-tick horizontal drift on partial-block shapes (stairs/slabs), walking the rider off the surface after ~80 ticks (solid decks were stable forever); vanilla collision has no such drift and, being a swept test, also handles fast falls without the OBB's adaptive sub-stepping.
applyContraptionMovement 把水平(只 yaw)船改回走 vanilla Entity.collideBoundingBox，OBB 只留給 pitch/roll 傾斜船。walk-sim gametest(solidDeckWalkNoFall vs stairsDeckWalkNoFall)顯示 OBB 的 collide-and-slide + push-out 在部分方塊(樓梯/半磚)上每 tick 累積水平漂移、~80 tick 後把人走出表面(實心甲板永遠穩定)；vanilla 碰撞沒這漂移，且是掃掠測試、快速墜落也不用 OBB 那套子步進。

obbCollideLocal sub-step count is now adaptive (ceil(|motion|/0.35) clamped 4..16) instead of a fixed 4, so a ~3.9-block/tick fall takes ~0.35-block sub-steps and the swept OBB test catches the deck before the box crosses it. Found by loading the real test ship and dropping a probe onto every walkable surface at -3.0/tick (19 tunnelled, including full quartz blocks and chests) versus zero after the change.
obbCollideLocal 子步數改成自適應（ceil(|motion|/0.35) 夾 4..16）而非固定 4，~3.9 格/tick 的墜落變成 ~0.35 格小步，掃掠 OBB 在盒跨過甲板前就抓到。靠載入真船、對每個可站面 -3.0/tick 丟 probe 診斷（修前 19 穿，含實心石英塊和箱子；修後 0）。

Player-versus-ship collision is now client-authoritative. EntityShipCollisionMixin and ShipDeckCarryHandler early-out when the entity is a Player on the server side (matching Create skipping PlayerType.SERVER); mobs stay server-authoritative. Two-sided [shipcol C/S] logging on applyContraptionMovement showed the server re-running it on the client's already-collided motion (server in = client out) and pushing the resting player up ~0.195/tick, fighting the client position and causing the clip-through-then-snap-back desync; after the fix the server logs zero player collisions while the client keeps running them.
玩家對船碰撞改成客戶端權威。EntityShipCollisionMixin 和 ShipDeckCarryHandler 在實體是伺服器端 Player 時提早 return（照 Create 跳過 PlayerType.SERVER）；生物維持伺服器權威。applyContraptionMovement 兩側 [shipcol C/S] log 顯示伺服器拿客戶端已碰撞的移動再跑一次（server in = client out）、每 tick 把站著的玩家往上推 ~0.195，跟客戶端位置打架造成「穿進去又被拉回」的 desync；修後伺服器端玩家碰撞歸零、客戶端照常。

飛船甲板碰撞改用移植自 Create 的 OBB 連續碰撞器。space.ship.collision（Matrix3d/OrientedBB/CollisionList/ContinuousOBBCollider）從 Create 移植（MIT；只改 package + catnip Iterate→boolean[]）；Matrix3d 加 set(Quaternionfc)。applyContraptionMovement 對所有船（不只傾斜）走 obbCollideLocal，因為 vanilla collideBoundingBox 在凸角落也會切角穿且不解析穿入。obbCollideLocal 子步進（N=4）：每段 連續 collide-and-slide（temporalResponse 移到撞擊點 + 去法線分量滑 + collisionResponse 推出）再做零移動 collideMany 的 MTV 推出迴圈；OBB 旋轉用 q0.conjugate，colliders 用碰撞形狀的快取 CollisionList（collisionListCache，跟 shape 一起失效）。組裝（placeAtShipCenter 後）+ readSpawnData 結尾呼叫 setOldPosAndRot，新船第 0 幀 xOld/yRotO 才正確（之前新船在錨於 0,0,0 的框碰撞）。makeBoundingBox 回傳以船 3D 中心為心、邊長=bounds 對角線的立方體，任何姿勢都點得到/碰得到。ShipOutlineRenderer 改用 ship.orientation(pt)（原本 yaw-only）。resolveOverlap 判定用半尺寸中心盒（只真穿入、非表面接觸），走甲板不會每 tick 被推；ShipDeckCarryHandler 在 isSupporting 時設 onGround + 清 fallDistance（船方塊不在世界裡，Create 也這樣）。診斷 gametest diagShipCollisionDirections/diagCornerMultiTick + stationaryShipBlocksHorizontalWalk/tiltedShipBlocksWalkOBB。已知殘留：upright 船的正對角線凸角點（measure-zero）還穿 ~1.0，單軸 MTV 推出在那收斂到仍穿入的不動點。效能：最多 N*(1+3) 次 collideMany/實體/tick，但 broad phase 對遠離船的實體早退。

Ship full-attitude + big-ship batch (mostly untested in-game). Six-axis flight: orientation() is a quaternion from yaw(getYRot)/pitch(getXRot)/roll(DATA_ROLL) plus the driver-seat bow offset (DATA_BOW) as rotateY(-lookYaw).rotateX(pitch).rotateZ(roll).rotateY(bow), so pitch/roll rotate about the bow axes regardless of build direction; A/D drives roll, the driver's look pitch drives ship pitch. Deck collision uses the full quaternion (q0 old-tick / q1 this-tick) with an enclosing-AABB of the player box (no fall-through at tilt) and a swept-box takeover gate lb.expandTowards(motion) (no fast-fall tunnelling); blockedBy ignores blocks already overlapping terrain (an embedded ship can escape); localCollisionShape fills empty-collision EntityBlocks solid. Big-ship performance: the dominant assembly freeze was per-block blockstate NBT in spawn serialization (41ms/110KB) -> ShipContraption.writeToBuf/readFromBuf write one varint global blockstate id per block via Block.getId/stateById (9ms/19KB, NBT kept for disk); addToWorld places structural blocks with UPDATE_CLIENTS|UPDATE_KNOWN_SHAPE (no neighbour cascade), EntityBlocks keep UPDATE_ALL; the hull VBO bakes off-thread (Util.backgroundExecutor over an immutable snapshot, GL upload on the render thread, old VBO kept until ready) with a debounced rebake; freshly placed blocks render per-frame (pendingVisualBlocks) until the VBO catches up. localCollisionShape uses a BitSetDiscreteVoxelShape (needs an access transformer for ArrayVoxelShape) and resolveTerrain iterates only hull blocks. MAX_BLOCKS 512->2048, footprint/height 32->48. Camera+rider tilt via ViewportEvent.ComputeCameraAngles (roll) + RenderPlayerEvent (pitch/roll in the body-yaw frame); third-person distance scaled by ship size in ShipCameraMixin (not vanilla getMaxZoom, which it overrides). Space: 8K earth day/cloud/night textures (STBImage loads any size) and a 2K tangent-space normal map on unit 3 with uHasNormal, perturbing the lit surface normal in atmosphere.fsh for relief. gametests perfBigShipPlacement / perfBigShipCollision; removed the flaky mock-player driverInputMovesShip.
飛船全姿勢 + 大船 batch（大半未實機驗）。六面飛行：orientation() 由 yaw/pitch/roll + 駕駛座船頭偏移(DATA_BOW)組四元數 rotateY(-lookYaw)·rotateX(pitch)·rotateZ(roll)·rotateY(bow)，pitch/roll 繞船頭軸轉、不受蓋的方向影響；A/D 驅動 roll、駕駛 look pitch 驅動船 pitch。甲板碰撞用完整四元數(q0 舊/q1 這 tick)+ 玩家盒外接 AABB(傾斜不穿)+ 掃掠盒接管判斷 lb.expandTowards(motion)(快速摔落不穿)；blockedBy 忽略已重疊地形的方塊(嵌進地形能脫離)；localCollisionShape 對空碰撞 EntityBlock 補滿格。大船效能：組裝凍結主因是 spawn 序列化每方塊編 blockstate NBT(41ms/110KB)→ writeToBuf/readFromBuf 每方塊一個 varint 全域 blockstate id(Block.getId/stateById，9ms/19KB，NBT 留存檔)；addToWorld 結構方塊用 UPDATE_CLIENTS|UPDATE_KNOWN_SHAPE(無鄰居連鎖)、EntityBlock 維持 UPDATE_ALL；VBO off-thread 烤(快照+GL 上傳在 render thread+舊 VBO 撐到烤好)+debounce；剛放方塊每幀先畫(pendingVisualBlocks)到 VBO 接棒。localCollisionShape 用 BitSetDiscreteVoxelShape(需 AT)、resolveTerrain 只迭代外殼。MAX_BLOCKS 512→2048、footprint/height 32→48。鏡頭+騎乘傾斜用 ComputeCameraAngles(roll)+RenderPlayerEvent(朝向框內 pitch/roll)；第三人稱距離在 ShipCameraMixin 依船大小縮放(非 vanilla getMaxZoom)。太空：8K 地球貼圖、2K 切線法線圖綁 unit3+uHasNormal 在 atmosphere.fsh 擾動受光法線做起伏。gametest perfBigShipPlacement/perfBigShipCollision；移除 flaky 的 driverInputMovesShip。

Mana alloy armor: extended the custom bedrock-parser armor model (non-GeckoLib) from helmet-only to the full four-piece set, all sharing one geometry file (mana_alloy_armor.geo.json) authored by Bksesame. The geometry is split by bone name into four groups: armorHead (helmet), armorBody+armorRightArm+armorLeftArm (chestplate), armorRightLeg+armorLeftLeg (leggings), and armorRightBoot+armorLeftBoot (boots). Boot bones were changed from leg-parented to standalone top-level bones (same pivot as the legs) so wearing leggings does not also render the boots and vice versa. ManaAlloyArmorModel now exposes per-slot poseXxx (copyFrom the matching vanilla humanoid part) and renderXxx methods; ManaAlloyArmorLayer renders only the bone group for each equipped ManaAlloy piece. Chestplate, leggings and boots items now also return the empty hiding model via the shared ManaAlloyArmorClientExtension in initializeClient (as the helmet already did). Visual-only, no gametest; needs runClient verification (leg pivot is 2 vs vanilla 1.9, may need micro offset).
魔力合金盔甲：把自訂 bedrock-parser 盔甲模型（非 GeckoLib）從只有頭盔擴成完整四件，四件共用同一個 geometry 檔（mana_alloy_armor.geo.json，蕎麥麵製作）。依 bone 名稱拆四組：armorHead（頭盔）、armorBody+armorRightArm+armorLeftArm（胸甲）、armorRightLeg+armorLeftLeg（護腿）、armorRightBoot+armorLeftBoot（靴子）。boot bone 從掛在 leg 下改成獨立 top-level bone（pivot 與腿相同），避免穿護腿時連靴一起畫、反之亦然。ManaAlloyArmorModel 改成每槽提供 poseXxx（copyFrom 對應 vanilla humanoid 部件）與 renderXxx；ManaAlloyArmorLayer 只畫每件已穿戴 ManaAlloy 部件對應的 bone 群。胸甲、護腿、靴子 item 現在也在 initializeClient 經共用的 ManaAlloyArmorClientExtension 回傳隱藏用空模型（頭盔早已如此）。純視覺、無 gametest；需 runClient 驗（leg pivot 為 2 而 vanilla 為 1.9，可能要微調 offset）。

Spaceship virtual world VM1-VM3 (machines run on ships via a shadow dimension). Rather than a fragile fake ServerLevel, the contraption blocks are copied into a hidden real dimension (koniava:ship_shadow) where they tick in a genuine ServerLevel, so block entities, capabilities and the mana conduit network all work for free; the player never enters it. VM1: ship_shadow registered (void, fixed noon for crops, no mob spawns); ShipShadowManager (SavedData) allocates a spaced region per ship, and assembly places the blocks there force-loaded (machines tick), cleared on disassembly/removal. VM2: tickServerMirror reflects shadow blockstate/BE-NBT changes (furnace lit, crops, machine progress) back onto the visual contraption and syncs to clients; ship edits also write to the shadow so the mirror does not revert them and the mana network sees them. VM3: interactAt forwards a BE block's use to the shadow block entity (real machine/chest GUI), with MenuShipShadowMixin making AbstractContainerMenu.stillValid pass when the menu's ContainerLevelAccess targets ship_shadow so the cross-dimension menu stays open. VM4: the shadow region is force-loaded only while the ship entity is loaded; ShipEntity.ensureShadowForceLoaded (re)adds the ticket each load and remove() drops it on chunk unload / dimension change (not on destroy), so far-away ships pause their machines and resume on reload. Remaining: BE-render machine visuals on the client (progress bars), and the moving-platform collision edge cases.
飛船虛擬世界 VM1-VM3（機器靠影子維度在船上運轉）。不用脆弱的假 ServerLevel，而是把 contraption 方塊複製進一個隱藏的真實維度（koniava:ship_shadow）在真 ServerLevel 裡 tick，所以 BE、capability、魔力 conduit 網路全部免費正常；玩家永不進入。VM1：註冊 ship_shadow（虛空、固定正午讓作物長、不生怪）；ShipShadowManager（SavedData）每船分配一個間隔區域，組裝把方塊 force-load 放進去（機器 tick），拆解/移除清掉。VM2：tickServerMirror 把影子的 blockstate/BE-NBT 變化（熔爐亮、作物、機器進度）鏡射回視覺 contraption 並同步 client；船的編輯也寫進影子，鏡射才不會蓋回去、魔力網路也看得到。VM3：interactAt 把 BE 方塊的 use 轉發給影子 BE（真機器/箱子 GUI），MenuShipShadowMixin 讓 AbstractContainerMenu.stillValid 在 menu 的 ContainerLevelAccess 指向 ship_shadow 時放行，跨維度選單不秒關。剩：VM4 force-load 生命週期（卸載閒置船）、client 端 BE-render 視覺。
...
Spaceship placement orientation via vanilla getStateForPlacement, and bed/double-plant multi-block. placeBlock now builds a BlockPlaceContext that runs the block's own getStateForPlacement, but with the player's clicked face and facing directions transformed into the ship's local frame (worldToLocalDir rotates a horizontal Direction by -shipYaw in 90-degree steps; Y unchanged) and getClickedPos forced to the target local position, so directional blocks (stairs, logs/AXIS, furnaces, etc.) orient exactly as vanilla would, in local space. The context still uses the real Level for neighbour/fluid reads, so auto-connecting blocks (fences, walls, panes) and waterlogging can be slightly off; orientation itself is correct. placeState applies the result and handles multi-cell blocks generically: DOUBLE_BLOCK_HALF blocks (doors, tall plants) place lower+upper, BedBlock places foot+head along FACING, with the second cell required free. The old default-orientation placeBlock(pos, block) was replaced; pickLocal now also returns the local hit Vec3 for slab/stair half. editPlacesDoorBothHalves gametest uses placeState.
飛船放置朝向用 vanilla getStateForPlacement，加床/高植物多方塊。placeBlock 現在建一個 BlockPlaceContext 跑方塊自己的 getStateForPlacement，但把玩家的點擊面與朝向轉進船的 local 框（worldToLocalDir 把水平 Direction 繞 -shipYaw 以 90° 整步旋轉，Y 不變），getClickedPos 強制成目標 local 位置，所以有方向的方塊（樓梯、原木 AXIS、熔爐等）會像 vanilla 一樣在 local 空間正確朝向。context 仍用真實 Level 讀鄰塊/流體，所以自動連接方塊（柵欄/牆/玻璃片）與含水可能略有偏差；朝向本身正確。placeState 套用結果並泛用處理多方塊：DOUBLE_BLOCK_HALF（門、高植物）放下+上、BedBlock 沿 FACING 放腳+頭，第二格要空。舊的預設朝向 placeBlock(pos, block) 已換掉；pickLocal 現在也回 local 命中 Vec3 給半磚/樓梯上下用。editPlacesDoorBothHalves gametest 改用 placeState。

Spaceship in-place editing on a parked ship (no disassembly). ShipContraption gained addBlock/removeBlock with bounds recompute. ShipEntity.updateContraptionBlock now unifies set/add/remove (air state removes, existing pos sets keeping NBT, new pos adds) and calls refreshDimensions since the bounds (and thus the hitbox) can change. interactAt, after the seat/door/chest dispatch, places the held BlockItem's default state into the empty local position adjacent to the picked face (pickLocal now returns the hit Direction too) when isParked() (no controlling passenger), consuming one item and syncing via ShipBlockUpdatePacket. hurt() is overridden so left-clicking a parked ship breaks the aimed block (drops it plus container Items, plays the break sound, syncs); the core at local (0,0,0) is protected. Both paths are gated on isParked. Multi-cell blocks are handled for doors: placeBlock places both DoubleBlockHalf halves (upper above) and canPlaceAt requires the upper cell free, and breaking a door half removes the other half too. editAddsAndRemovesBlock and editPlacesDoorBothHalves gametests cover it.
飛船停船原地編輯（不用拆解）。ShipContraption 加了 addBlock/removeBlock 並重算 bounds。ShipEntity.updateContraptionBlock 現在統一 set/add/remove（air 移除、已存在位置換 state 保留 NBT、新位置新增），並呼叫 refreshDimensions 因為 bounds（進而 hitbox）會變。interactAt 在座椅/門/箱子分流之後，當 isParked()（沒人駕駛）時把手持 BlockItem 的預設 state 放到命中面相鄰的空 local 位置（pickLocal 現在也回命中 Direction），扣一個物品並用 ShipBlockUpdatePacket 同步。覆寫 hurt()，左鍵停著的船=挖掉指到的方塊（掉落+容器 Items、播破壞音、同步）；核心 local (0,0,0) 受保護。兩條路都 gate 在 isParked。editAddsAndRemovesBlock gametest 蓋加/刪 + size/bounds 變化。
...
Spaceship per-seat interaction, seat picking, pilot seats, and dismount. ShipEntity.interactAt now raycasts the player's look into the contraption's local block shapes (pickLocalBlock clips each block's outline shape in local space) and dispatches per block: a seat boards, anything else (the ship body) passes, leaving a TODO hook for forwarding to doors/chests/machines. Seat assignment is explicit and synced: a SynchedEntityData CompoundTag maps passenger UUID to seat index, set to the clicked seat on board (or the first free seat if taken) and cleared on dismount; seatIndexOf reads it (falling back to passenger-list order). positionRider, getControllingPassenger, the input slots, and dismount all key off this assigned seat. The two seats nearest the core (index < MAX_DRIVERS=2) are pilot seats; getControllingPassenger returns the occupant of the lowest occupied pilot seat (not the first rider), ShipInputPacket rejects input from non-pilot seats, and tickServerMovement takes the heading/bow and throttle from the actual primary pilot seat (could be seat 1). On board the player's view is snapped to the seat's bow direction via ServerPlayer.connection.teleport so the look-following ship does not swing to face them. removePassenger places the dismounting player at the first of several candidate spots around the seat that is not inside the ship (isInsideShip), else above it. ShipSeatBlock got getShape/getCollisionShape matching the chair model (seat slab + backrest), rotated per FACING to match horizontalBlock; getStateForPlacement and ShipEntity.sitYawLocal both dropped the erroneous getOpposite so the bow aligns with the visible chair (fixes the reversed view). SEAT_SIT_HEIGHT lowered 0.4 -> 0.0 (the riding sit pose already raises the body).
飛船依座位互動、選座位、駕駛位、下船。ShipEntity.interactAt 現在把玩家視線 raycast 進 contraption 的 local 方塊形狀（pickLocalBlock 在 local 空間逐方塊 clip outline），依方塊分流：座椅=上船，其他（船身）=PASS，留 TODO 給門/箱子/機器轉發。座位指派明確且同步：SynchedEntityData CompoundTag 存乘客 UUID→座位 index，上船時設成點到的座位（被佔則第一個空位）、下船清掉；seatIndexOf 讀它（退回上船順序）。positionRider、getControllingPassenger、輸入槽、下船都依這個指派的座位。離核心最近的兩張椅子（index < MAX_DRIVERS=2）是駕駛位；getControllingPassenger 回最小有人駕駛位的人（非第一個上船者），ShipInputPacket 擋掉非駕駛位的輸入，tickServerMovement 的船頭/油門取自實際主駕駛位（可能是座位1）。上船時用 ServerPlayer.connection.teleport 把視角 snap 到座位船頭方向，跟視角轉的船才不會甩去面向你。removePassenger 把下船玩家放到座位周圍第一個不在船體內的候選點（isInsideShip），都不行就放上方。ShipSeatBlock 加了對齊椅子模型的 getShape/getCollisionShape（座板+椅背），依 FACING 旋轉對齊 horizontalBlock；getStateForPlacement 與 ShipEntity.sitYawLocal 都去掉錯誤的 getOpposite，讓船頭對齊視覺椅子（修視角反）。SEAT_SIT_HEIGHT 從 0.4 降到 0.0（騎乘坐姿本身會抬高身體）。

Spaceship deck collision and carry (stand and walk on a moving ship). Ship blocks live on the entity, not the world, so external entities used to fall through. ShipEntity now builds a cached local-frame VoxelShape (union of block collision shapes) and exposes restrictMotion(worldBox, worldMotion): it transforms the entity box and motion into the ship's local frame (inverse of rotatedWorldPoint; player box approximated axis-aligned since only yaw rotates), runs per-axis VoxelShape.collide (Y first so you stand, then X/Z for walls), and rotates the result back to world. EntityShipCollisionMixin injects at the return of Entity.collide and further restricts the world-resolved motion by nearby ships, so collision is correct even when the ship is rotated. Carry: ShipDeckCarryHandler (EntityTickEvent.Post, both sides) detects entities standing on a ship via isSupporting (a thin probe just below the feet intersects the local shape) and applies carry(): the ship's per-tick translation plus a rotation of the entity's offset around the ship center by the yaw delta (xOld/yOld/zOld/yRotO from setOldPosAndRot), and adds the yaw delta to the entity's rotation. Seat fix: positionRider lowered the rider from +1.0 (full-block top, floated above the chair) to SEAT_SIT_HEIGHT 0.4 and sets the rider yBodyRot to the seat's sit direction (sitYawLocal = FACING.getOpposite()) so the body faces forward instead of the vanilla default; bowLocalYaw now delegates to the generalized sitYawLocal. The collision math (floor stop + wall block) is covered by the deckCollisionStopsFall gametest.
飛船甲板碰撞與 carry（站上會動的船走動）。船方塊在實體上不在世界裡，外部實體原本會穿過去。ShipEntity 現在建一個快取的 local 框 VoxelShape（方塊碰撞盒合併）並開放 restrictMotion(worldBox, worldMotion)：把實體盒與移動轉進船的 local 框（rotatedWorldPoint 的逆；只有 yaw 旋轉故玩家盒近似軸對齊），逐軸跑 VoxelShape.collide（Y 先=站得住、再 X/Z=牆），結果轉回世界。EntityShipCollisionMixin 注入 Entity.collide 的回傳，再用附近飛船限制世界解算後的移動，所以船旋轉時碰撞也正確。Carry：ShipDeckCarryHandler（EntityTickEvent.Post，兩側）用 isSupporting（腳底薄探針與 local 形狀相交）判定站在船上，套用 carry()：船這 tick 的平移 + 實體相對船中心的位移依 yaw delta 旋轉（xOld/yOld/zOld/yRotO 來自 setOldPosAndRot），並把 yaw delta 加到實體朝向。座椅修正：positionRider 把乘客從 +1.0（整格頂、浮在椅上）降到 SEAT_SIT_HEIGHT 0.4，並把 yBodyRot 設成座椅坐姿方向（sitYawLocal = FACING.getOpposite()）讓身體面向前而非 vanilla 預設；bowLocalYaw 改委派給通用的 sitYawLocal。碰撞數學（地板擋落、牆擋移動）由 deckCollisionStopsFall gametest 覆蓋。

Spaceship bow now comes from the driver seat facing (build in any orientation), driver seats, and seat rotation on disassembly. Bow was hardcoded to local -Z (the north side of the build, because the contraption local frame equals the world frame at assembly), so you had to build facing north or the ride orientation was wrong. The bow is now bowLocalYaw(), derived from the nearest-to-core seat's FACING (sit direction = FACING.getOpposite() due to the north-backrest model + horizontalBlock), so orienting the driver chair defines forward and any build orientation works; heading lerps to driverLook - bowLocalYaw and movement is taken relative to the visible bow (bowLocal + shipYaw). The driver seat changed from the core to the nearest seat block (the core is the control block, only used as a fallback driver when there are no seats); seats are sorted by distance to the core, the first MAX_DRIVERS (2) are driver seats and the rest are unlimited passengers. Up to two people can pilot at once: each driver seat has its own input slot, the throttle (forward/strafe/vertical) sums and clamps across both, and the heading follows the primary driver (seat 0). ShipInputPacket now carries no seat index; the server derives it from the player's passenger index and rejects non-driver-seat input, and the client only sends from a driver seat. ShipSeatBlock gained rotate/mirror overrides so the chair facing rotates with the ship when disassembly writes the blocks back (HorizontalDirectionalBlock does not rotate FACING by default).
飛船船頭改由駕駛椅朝向決定（可朝任意方位建造）、駕駛位、拆解時座椅跟轉。船頭原本寫死 local -Z（建造物的北側，因為 contraption local 座標 = 組裝當下世界座標），所以必須朝北建造否則騎乘方位錯。現在船頭是 bowLocalYaw()，由離核心最近座椅的 FACING 推出（坐姿 = FACING.getOpposite()，因椅背在北的模型 + horizontalBlock），所以把駕駛椅轉去哪就是船頭、任意方位建造都行；朝向 lerp 到 driverLook - bowLocalYaw、移動相對可見船頭（bowLocal + shipYaw）算。駕駛位從核心改成最近的座椅方塊（核心是控制方塊，只在完全沒座椅時當後備駕駛）；座椅依離核心距離排序，前 MAX_DRIVERS（2）張是駕駛位、其餘是不限量乘客。最多兩人同時駕駛：每個駕駛位一個輸入槽，油門（前後/左右/升降）兩位相加夾值，船頭跟隨主駕駛（座位0）。ShipInputPacket 不再帶座位 index；server 從玩家乘客 index 推出並拒絕非駕駛位輸入，client 只在駕駛位才送。ShipSeatBlock 加了 rotate/mirror 覆寫，拆解寫回方塊時椅子朝向會跟船一起轉（HorizontalDirectionalBlock 預設不旋轉 FACING）。

Spaceship movement switched to server authority with client-side interpolation, fixing the move-plus-turn jitter and all position desync. The ship had been client-authoritative (the driver predicted movement and pushed its position to the server), which fought the server's entity-tracking updates and left the rider camera and ship render on two different interpolation paths, so moving while turning jittered and dismounts desynced. Now the driver only sends input (new ShipInputPacket: forward/strafe/vertical/yaw, replacing the old position-carrying ShipMovePacket), the server is the sole authority and runs the movement in tick (server side only), and every client including the driver follows the server position through standard entity tracking. ShipEntity.lerpTo stores the target and ShipEntity.tick steps toward it via lerpPositionAndRotationStep on the client (Entity.lerpSteps is private with no getter, so a local copy is kept), exactly like a vanilla boat. The client prediction branch, the ShipMovePacket position sync, the dismount final-sync, and the lerpTo skip-while-controlling override were all removed. SHIP updateInterval lowered from 3 to 1 so the server broadcasts position every tick for the smoothest driven-vehicle follow.
飛船移動改成伺服器權威 + client 端插值，修掉移動+轉向的抖動與所有位置不同步。船原本是 client 權威（駕駛預測移動並把位置推給 server），會跟 server 的 entity tracking 更新打架，且乘客相機與船身渲染走兩條不同插值路徑，所以邊移動邊轉就抖、下船會不同步。現在駕駛只送輸入（新 ShipInputPacket：forward/strafe/vertical/yaw，取代舊的帶位置的 ShipMovePacket），server 是唯一真相、在 tick（僅 server 端）算移動，所有 client 含駕駛都經標準 entity tracking 跟隨 server 位置。ShipEntity.lerpTo 存目標、ShipEntity.tick 在 client 用 lerpPositionAndRotationStep 步進（Entity.lerpSteps 私有無 getter，自存一份），就像 vanilla 船。移除了 client 預測分支、ShipMovePacket 位置同步、下船補送、以及 lerpTo 駕駛期間略過的覆寫。SHIP updateInterval 從 3 降到 1，server 每 tick 廣播位置，駕駛跟隨最順。

Spaceship rider seating, bow direction, base collision, and dismount sync fixed. Rider sat on air when the ship was rotated: positionRider added the +0.5/+0.5 centering offset in world space after rotation, so it did not rotate with the ship; the offset is now folded into the local point before rotation via a new rotatedWorldPoint(double,double,double) that rotatedWorldCorner delegates to. Bow direction: the core (driver seat, local -Z) is now the bow, so the driver faces outward like a horse instead of the deck facing back at them; the heading lerps to riderYaw+180 and forward/right were re-derived (sin,-cos)/(cos,sin) so W still moves toward the look. Assembly stays untouched (resting yaw 0 = built orientation). Base collision: isShipScaffolding now skips only the gantry (so the ship can rise through its frame); the assembly base and pad are solid again instead of being phased through everywhere. Dismount sync: the driver client predicts movement and sends ShipMovePacket each tick, but after dismount the ship is stationary so the server stops broadcasting and the last client/server gap never closed; a final ShipMovePacket is now sent on the controlling->uncontrolled transition, and lerpTo is ignored while isControlledByLocalInstance so the driver's prediction is not pulled back by server updates.
飛船乘客落座、船頭方向、底座碰撞、下船同步修復。船旋轉時乘客坐到空氣上：positionRider 把 +0.5/+0.5 置中偏移在旋轉後於世界座標加，沒跟船轉；改用新的 rotatedWorldPoint(double,double,double)（rotatedWorldCorner 也委派給它）在旋轉前併入 local。船頭方向：核心（駕駛位、local -Z）現在是船頭，駕駛朝外開（像馬頭在前），不再是甲板朝向自己；朝向 lerp 到 riderYaw+180，前進/右手重新推導為 (sin,-cos)/(cos,sin)，W 仍往視角走。組裝不動（靜止 yaw 0=建造朝向）。底座碰撞：isShipScaffolding 現在只跳過組裝架（船能從框架升起），底座與組裝台恢復實心、不再到處被穿透。下船同步：駕駛 client 預測移動並每 tick 送 ShipMovePacket，但下船後船靜止、server 不再廣播，最後的 client/server 落差永遠不收斂；現在在 controlling→uncontrolled 轉換時補送一發 ShipMovePacket，且 isControlledByLocalInstance 期間忽略 lerpTo，駕駛的預測不被 server 更新拉回。

Spaceship turn-skew, reversed strafe, and seat facing fixed. Turn-skew: the ship moves by setPos/setYRot directly, so xOld/yOld/zOld and yRotO were never updated; the renderer interpolates both position and yaw by partialTick while collision and seating use the live getX/getYRot, so the render pivot diverged from collision and the whole ship looked skewed once yaw left 0. tick() now calls setOldPosAndRot() at the top so both old position and old rotation track, keeping render interpolation aligned with collision. Reversed strafe: rightDir was (cos, sin), which points east at yaw 0, but facing south your right is west; corrected to (-cos, -sin). Seat facing: the chair model has its backrest on the north face and horizontalBlock applies (toYRot+180)%360, so the seated orientation is the opposite of the stored FACING; getStateForPlacement now stores getHorizontalDirection().getOpposite() so you sit facing the way you were looking when you placed it (only affects newly placed seats).
飛船轉向歪掉、左右相反、座椅朝向修復。轉向歪：船用 setPos/setYRot 直接移動，xOld/yOld/zOld 與 yRotO 從不更新；渲染對位置和 yaw 都做 partialTick 插值，但碰撞與座位用當前 getX/getYRot，於是渲染支點與碰撞分家，yaw 一離開 0 整艘就歪。tick() 現在開頭呼叫 setOldPosAndRot()，位置與角度舊值都對齊，渲染插值與碰撞一致。左右相反：rightDir 原為 (cos, sin)，yaw 0 時指東，但朝南時右邊是西；改成 (-cos, -sin)。座椅朝向：椅子模型椅背在北面、horizontalBlock 套 (toYRot+180)%360，坐姿是 FACING 的反向；getStateForPlacement 改存 getHorizontalDirection().getOpposite()，讓坐姿朝你放置時看的方向（只影響新放的座椅）。

Spaceship hitbox re-centered on the ship center so the clickable box hugs the actual blocks instead of being oversized. EntityDimensions is a symmetric box (centered on X/Z, feet up), but the entity origin used to sit at the core corner, so the box had to span the full reach in every direction and ended up much larger than the ship. The entity origin now sits at the ship center (X/Z midpoint, Y bottom) via centerOffset() and placeAtShipCenter(); getDimensions returns the tight footprint (larger of width/depth, ship height). rotatedWorldCorner subtracts centerOffset before rotating, so collision, seats, and disassembly all measure from the center; the renderer rotates around the entity origin then translate(-centerOffset) to keep blocks aligned; disassembly derives the core anchor from rotatedWorldCorner(0,0,0); getBoundingBoxForCulling shifts bounds by -centerOffset. All 12 ship gametests pass.
飛船碰撞箱重新對齊到船中心，讓可點擊框貼合實際方塊不再過大。EntityDimensions 是對稱盒（X/Z 置中、腳底往上），但實體原點以前在核心角落，盒子得往每個方向涵蓋最大延伸，結果遠大於船。實體原點現在用 centerOffset() 與 placeAtShipCenter() 放在船中心（X/Z 中點、Y 底）；getDimensions 回傳貼合 footprint（寬深取大者、船高）。rotatedWorldCorner 旋轉前先減 centerOffset，碰撞、座椅、收船都從中心量；渲染器繞實體原點旋轉後 translate(-centerOffset) 維持方塊對齊；收船從 rotatedWorldCorner(0,0,0) 反推核心錨點；getBoundingBoxForCulling 把 bounds 平移 -centerOffset。飛船 12 個 gametest 全綠。

Spaceship made server-authoritative; fixes the render-vs-actual position split and the assembly flip. Two bugs: after flying and dismounting the ship snapped back to the assembly spot visually while disassembly placed blocks at the flown spot (the render and the real server position had diverged), and the ship flipped 180 degrees the moment it was assembled. Root cause of the position split: the ship had been driven client-authoritatively, relying on vanilla ServerboundMoveVehiclePacket to sync, which was unreliable. ShipEntity now overrides isControlledByLocalInstance to false so the client never sends that vehicle packet; the driver client sends only input (ShipControlPacket, re-added), the server computes movement in tick and is the single source of truth, and the position syncs to every client through normal entity tracking. The 180-degree render/collision/disassembly offset added in the previous attempt was reverted, so an assembled ship renders exactly as it was built (yaw 0 = identity). The flight gametest that relied on a mock player keeping its seat across a long move was removed (finicky in gametest, not a real-game issue) and the terrain-collision test now drives resolveTerrain directly without riding.
飛船改成 server 權威；修「渲染位置 vs 實際位置分家」與「組裝翻 180」。兩個 bug：飛走+下船後畫面上船彈回組裝點，但拆解卻在飛走的位置放方塊（渲染與 server 真實位置分家了），以及船一組裝就翻 180°。位置分家根因：船原本用「駕駛 client 為位置權威」靠 vanilla ServerboundMoveVehiclePacket 同步，不可靠。ShipEntity 現在覆寫 isControlledByLocalInstance 回 false，client 永不送那個載具封包；駕駛 client 只送輸入（ShipControlPacket，重新加回），server 在 tick 算移動當單一真相，位置經一般 entity tracking 同步給每個 client。前一版加的 180° 渲染/碰撞/收船偏移已退回，組裝好的船照建造原樣渲染（yaw 0=不轉）。移除了依賴 mock player 長距離保持座位的飛行 gametest（gametest 不穩，非真實遊戲問題），撞地形測試改直接驅動 resolveTerrain、不騎乘。

Spaceship full yaw rotation (first cut, needs in-game tuning). The ship now turns the whole craft to face the driver look direction, like the Ender Dragon banking to its flight path. ShipEntity.tick lerps the entity yaw toward the driver yaw (YAW_LERP) and computes movement relative to the ship yaw; rotatedWorldCorner rotates each block local corner around the core center (X/Z only), used by both collision (axis-aligned 1x1 approximation at the rotated position, no OBB) and the seats (positionRider seats riders at rotated positions and turns non-driver riders with the ship). The renderer rotates the whole contraption (VBO, per-frame, and BlockEntity passes) around the core center by Axis.YP.rotationDegrees(-yaw) using the interpolated view yaw. Because blocks can only be placed grid-aligned, disassembly snaps the yaw to the nearest 90 degrees and ShipContraption.addToWorld now rotates both the block positions and the blockstates (state.rotate) by that Rotation. Fixed a pre-existing bug in the ship-stops-at-terrain gametest that compared the absolute z to a small constant instead of the relative displacement.
飛船整艘 yaw 轉向（第一版，需實機微調）。船現在整艘轉去面向駕駛的視角方向，像終界龍朝飛行路徑傾轉。ShipEntity.tick 把實體 yaw 朝駕駛 yaw 平滑（YAW_LERP），移動相對船頭算；rotatedWorldCorner 把每個方塊 local 角落繞核心中心旋轉（只 X/Z），給碰撞（旋轉後位置的軸對齊 1x1 近似，不做 OBB）與座椅（positionRider 把乘客放到旋轉後座位、非駕駛乘客視角跟船轉）共用。渲染器用插值後的視角 yaw、Axis.YP.rotationDegrees(-yaw) 把整艘 contraption（VBO/逐方塊/BlockEntity）繞核心中心轉。因為方塊只能對齊格放回，收船把 yaw snap 到最近 90°，ShipContraption.addToWorld 現在把方塊位置與 blockstate（state.rotate）一起套該 Rotation。修了 ship-stops-at-terrain gametest 一個既有 bug（拿絕對 z 比小常數，應比相對位移）。

Spaceship seat facing and gametest-compatible movement guard. The ship seat is now a HorizontalDirectionalBlock with a FACING property set from the placer (getHorizontalDirection opposite), and its datagen uses horizontalBlock so the chair model rotates; the facing is captured in the contraption blockstate and rendered rotated on the ship. Movement guard: ShipEntity.tick computes movement when isControlledByLocalInstance() OR on the server. The driver client is the authority (it sends ServerboundMoveVehiclePacket); the real server runs the same branch but with input 0 (input is only set on the local driver client), so it is a no-op overwritten by the client position. GameTests have no real client (the mock player is not local), so the server branch drives movement there from the test-set input, keeping the flight/terrain tests working.
飛船座椅朝向與 gametest 相容的移動 guard。座椅改成 HorizontalDirectionalBlock 加 FACING（放置時取玩家朝向的反向），datagen 用 horizontalBlock 讓椅子模型旋轉；朝向存進 contraption blockstate，在船上也渲染成轉好的方向。移動 guard：ShipEntity.tick 在 isControlledByLocalInstance() 或 server 端算移動。駕駛 client 是權威（送 ServerboundMoveVehiclePacket）；真實 server 也跑同一段但輸入=0（輸入只在駕駛 client 本地設），等於 no-op 被 client 位置覆蓋。GameTest 沒有真 client（mock player 非 local），所以那邊由 server 端用測試設的輸入驅動移動，飛行/撞地形測試才能繼續用。

Spaceship movement fix, seat model, and pad GUI rework. Movement fix: an assembled ship could not move because its own launch structure (assembly base/gantry/pad, still in the world) counted as terrain and blocked it. ShipEntity collision now iterates the overlapping world blocks per ship cell and skips ship scaffolding (base/gantry/pad/core/seat), so the ship can fly out of its own frame; it still collides with real terrain. Seat model: ship_seat now uses a custom chair model (seat slab + backrest, built in ModBlockStateProvider via the element API) with a redrawn cushion+metal-trim texture, instead of a full cube. Pad GUI: the Assemble button now sits above Scan on the left and Scan is nudged left; a new Disassemble button on the right panel sends ShipDisassemblePacket, which parks the nearest ShipEntity within 16 blocks of the pad (sneak + right-click on the ship still works too, for ships away from a pad).
飛船移動修復、座椅模型、組裝台 GUI 改版。移動修復：組裝好的船動不了，因為它自己的發射台結構（組裝底座/組裝架/組裝台，還留在世界裡）被當成地形擋住。ShipEntity 碰撞現在逐船格迭代重疊的世界方塊，跳過飛船鷹架（底座/組裝架/組裝台/核心/座椅），船因此能飛出自己的框架；仍會撞真實地形。座椅模型：ship_seat 改用自訂椅子模型（座板 + 椅背，在 ModBlockStateProvider 用 element API 建）加重畫的軟墊+金屬邊貼圖，不再是整塊立方。組裝台 GUI：組裝按鈕移到掃描上方、掃描往左挪；右側面板新增拆解按鈕送 ShipDisassemblePacket，收回組裝台 16 格內最近的飛船（潛行+右鍵船身仍可用，給遠離組裝台的船）。

Spaceship VBO render fix and gantry texture: fixed the baked-VBO path drawing the ship stuck to the camera. The bake stores contraption-local vertices and the entity render PoseStack only carries the entity-relative-to-camera translation (the camera rotation lives in the global modelview), so ShipMeshCache.draw now uses RenderSystem.getModelViewMatrix() multiplied by pose.last().pose() as the model-view, instead of the pose alone. Redrew the ship_assembly_gantry texture as a metal truss (side rails + X cross-bracing) instead of hazard stripes.
飛船 VBO 渲染修復與組裝架貼圖：修正烤好的 VBO 把船黏著相機畫的問題。烤的是 contraption local 頂點，而實體渲染的 PoseStack 只帶「實體相對相機的位移」（相機旋轉在全域 modelview），所以 ShipMeshCache.draw 現在用 RenderSystem.getModelViewMatrix() 乘上 pose.last().pose() 當 model-view，而不是只用 pose。組裝架貼圖重畫成金屬桁架（側邊鋼樑 + X 交叉支撐），不再是警示條紋。
