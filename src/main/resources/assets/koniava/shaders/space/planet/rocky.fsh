...
mat3 buildFrame(vec3 z) {
    ...
    return transpose(mat3(x, cross(z, x), z));
}
...
void main() {
    ...
    if (hit.x > 0.0 && hit.x < hit.y) {
        ...
        vec3  n   = normalize(p);
        vec3  sun = normalize(vec3(-4.0, 1.0, 2.0));
        float li  = max(dot(n, sun), 0.0) * 0.65 + 0.35;

        vec2  uv  = vec2(atan(n.x, n.z) / (2.0 * PI) + 0.5,
                         clamp(acos(clamp(n.y, -1.0, 1.0)) / PI, 0.0, 1.0));
        float c   = fbm(uv * 4.0);
        vec3  col = li * mix(uColorLight, uColorDark, smoothstep(0.3, 0.7, c));
        col      += c * uHeatAmount * uHeatColor;
        col       = pow(max(col, 0.0), vec3(1.0 / 2.2));
        // pre-multiplied alpha 避免邊緣漏背景
