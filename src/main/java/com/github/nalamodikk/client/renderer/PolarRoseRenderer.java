...
/**
 * Polar rose r = cos(k * theta). k cycles 1..7 every 80 ticks.
 */
public class PolarRoseRenderer {

    private static final int   SEGS      = 1000;
    private static final float SCALE     = 2.5f;
    private static final int   CYCLE     = 80;
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        for (ActiveEffect eff : effects) {
            ...
            int k = 1 + ((int)(dt / CYCLE) % 7);
            float tMax = (float)(2 * Math.PI);

            buf.clear();
            int vertCount = 0;
            for (int i = 0; i < SEGS; i++) {
                float t0 = i       * tMax / SEGS;
                float t1 = (i + 1) * tMax / SEGS;
                float r0 = SCALE * (float)Math.cos(k * t0);
                float r1 = SCALE * (float)Math.cos(k * t1);
                float[] c = hue((float)i / SEGS);
                buf.put(ox + r0 * (float)Math.cos(t0)).put(oy).put(oz + r0 * (float)Math.sin(t0))
                   .put(c[0]).put(c[1]).put(c[2]).put(0.9f);
                buf.put(ox + r1 * (float)Math.cos(t1)).put(oy).put(oz + r1 * (float)Math.sin(t1))
                   .put(c[0]).put(c[1]).put(c[2]).put(0.9f);
                vertCount += 2;
            }
            buf.flip();
    ...
    private static void doInit() {
        ...
        try {
            ...
            GL20.glUseProgram(0);
        } catch (Exception e) { KoniavacraftMod.LOGGER.error("[PolarRose] Shader load failed", e); initFailed = true; return; }
