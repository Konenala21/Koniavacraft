...
public class AltarExplosionRenderer {

    private static final String[] FRAG_PATHS = {
        "shaders/altar/common_explosion.glsl",
        "shaders/altar/fx_shockwave.glsl",
        "shaders/altar/stage_altar_explosion.fsh",
    };
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        List<Map.Entry<BlockPos, Float>> entries = AltarExplosionManager.getActive();
        ...
        GL20.glUniform1f(locPillarAlpha, 0f);
        GL20.glUniform1f(locPillarRadius, 0f);
        GL20.glUniform1f(locGroundTwist, 0f);

        GL30.glBindVertexArray(vaoId);
        for (var entry : entries) {
            BlockPos pos  = entry.getKey();
            float    tick = entry.getValue();
    ...
    // 3 waves: start 15 ticks apart, each expands to 64 blocks over 60 ticks
    ...
    private static float[] computeWaves(float tick) {
        ...
        for (int w = 0; w < 3; w++) {
            float wAge = tick - w * 15f;
            if (wAge <= 0f || wAge > 60f) continue;
            float progress  = wAge / 60f;
            data[w * 3]     = progress * 64f;
            data[w * 3 + 1] = (float) Math.sin(progress * Math.PI) * 0.9f;
            data[w * 3 + 2] = 1.5f + (1f - progress) * 1.0f;
        }
        return data;
    }
    ...
    private static void init() {
        ...
        try {
            ...
            if (GL20.glGetProgrami(programId, GL20.GL_LINK_STATUS) == GL11.GL_FALSE) {
                KoniavacraftMod.LOGGER.error("[ExplosionRenderer] Link: {}", GL20.glGetProgramInfoLog(programId));
            ...
            KoniavacraftMod.LOGGER.error("[ExplosionRenderer] Init failed", e);
    ...
    private static int compile(int type, String src) {
        ...
        if (GL20.glGetShaderi(id, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            KoniavacraftMod.LOGGER.error("[ExplosionRenderer] Compile: {}", GL20.glGetShaderInfoLog(id));
            initFailed = true;
        }
        return id;
    }
