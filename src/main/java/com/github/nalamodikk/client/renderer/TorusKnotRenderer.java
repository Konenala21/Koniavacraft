...
/**
 * Torus knot (p=2, q=3) rotating in 3D.
 * x=(R+r*cos(q*t))*cos(p*t), y=r*sin(q*t), z=(R+r*cos(q*t))*sin(p*t)
 */
public class TorusKnotRenderer {

    private static final int   SEGS      = 400;
    ...
    private static final int   FLOAT_CAP = SEGS * 2 * 7 + 64;
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        for (ActiveEffect eff : effects) {
            ...
            float oy  = (float)(eff.pos.y + 1.5 - cam.y);
            ...
            for (int i = 0; i < SEGS; i++) {
                float t0 = i       * (float)(2 * Math.PI) / SEGS;
                float t1 = (i + 1) * (float)(2 * Math.PI) / SEGS;

                float[] v0 = knotPoint(t0, rot);
                float[] v1 = knotPoint(t1, rot);
                float[] c = hue((float)i / SEGS);

                buf.put(ox + v0[0]).put(oy + v0[1]).put(oz + v0[2]).put(c[0]).put(c[1]).put(c[2]).put(0.95f);
                buf.put(ox + v1[0]).put(oy + v1[1]).put(oz + v1[2]).put(c[0]).put(c[1]).put(c[2]).put(0.95f);
                vertCount += 2;
            }
            buf.flip();
    ...
    private static float[] knotPoint(float t, float yRot) {
        float rT = R + r * (float)Math.cos(Q * t);
        float wx = rT * (float)Math.cos(P * t);
        float wy = r  * (float)Math.sin(Q * t);
        float wz = rT * (float)Math.sin(P * t);
        // rotate around Y
    ...
    private static void doInit() {
        ...
        try {
            ...
            GL20.glUseProgram(0);
        } catch (Exception e) { KoniavacraftMod.LOGGER.error("[TorusKnot] Shader load failed", e); initFailed = true; return; }
