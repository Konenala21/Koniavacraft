...
/**
 * Expanding concentric ripple rings in the XZ plane.
 * Each effect emits multiple rings that expand outward and fade.
 */
public class RippleRenderer {

    private static final float MAX_RADIUS   = 5.0f;
    private static final float EXPAND_TICKS = 60.0f;   // ticks to reach MAX_RADIUS
    private static final int   EMIT_INTERVAL = 12;      // new ring every 12 ticks
    private static final int   MAX_RINGS     = 8;
    private static final int   RING_SEGS     = 72;
    private static final int   FLOAT_CAP     = (RING_SEGS * 2 + 4) * MAX_RINGS * 7 * 4;

    private static class RingEffect {
        final Vec3   pos;
        final long   startTick;
        int          lastEmit = Integer.MIN_VALUE;
        RingEffect(Vec3 pos, long startTick) { this.pos = pos; this.startTick = startTick; }
    }

    private static final List<RingEffect> effects = new ArrayList<>();
    ...
    public static void spawnEffect(Vec3 pos, long tick) {
        ...
        effects.add(new RingEffect(pos, tick));
    }
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        for (RingEffect eff : effects) {
            ...
            // Draw rings at phases: each ring starts at a multiple of EMIT_INTERVAL
            for (int phase = 0; phase <= (int)(dt / EMIT_INTERVAL); phase++) {
                float ringAge = dt - phase * EMIT_INTERVAL;
                if (ringAge < 0) continue;
                float progress = ringAge / EXPAND_TICKS;
                if (progress > 1.0f) continue;

                float r     = MAX_RADIUS * progress;
                float alpha = (1.0f - progress) * 0.85f;

                // Color cycles through cyan → blue → purple as it expands
                float hue = progress;
                float cr = 0.1f + hue * 0.6f;
                float cg = 1.0f - hue * 0.8f;
                float cb = 1.0f;

                for (int s = 0; s < RING_SEGS; s++) {
                    float a0 = s       * (float)(2 * Math.PI) / RING_SEGS;
                    float a1 = (s + 1) * (float)(2 * Math.PI) / RING_SEGS;
                    buf.put(ox + r*(float)Math.cos(a0)).put(oy).put(oz + r*(float)Math.sin(a0))
                       .put(cr).put(cg).put(cb).put(alpha);
                    buf.put(ox + r*(float)Math.cos(a1)).put(oy).put(oz + r*(float)Math.sin(a1))
                       .put(cr).put(cg).put(cb).put(alpha);
                    vertCount += 2;
                }
            }
    ...
    private static void doInit() {
        ...
        try {
            ...
            GL20.glUseProgram(0);
        } catch (Exception e) { KoniavacraftMod.LOGGER.error("[Ripple] Shader load failed", e); return; }
