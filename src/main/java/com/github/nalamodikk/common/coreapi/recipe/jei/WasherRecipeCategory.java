 ...
 * 💧 清洗機 JEI 分類顯示
 *
 * 在 JEI 中顯示清洗機配方的佈局和邏輯
 */
public class WasherRecipeCategory implements IRecipeCategory<ProcessingRecipe> {

    public static final RecipeType<ProcessingRecipe> RECIPE_TYPE =
            RecipeType.create(KoniavacraftMod.MOD_ID, "washer", ProcessingRecipe.class);
    ...
    public WasherRecipeCategory(IGuiHelper guiHelper) {
        // 圖標（用於分類選擇面板）
        // TODO: 改為 ModBlocks.ORE_WASHER.get() 當 Block 註冊後
        this.icon = guiHelper.createDrawableIngredient(
                VanillaTypes.ITEM_STACK,
                new ItemStack(Items.WATER_BUCKET) // 暫時用水桶代替
        );

        this.localizedName = Component.translatable("jei.koniava.washer");
    }
