 ...
 * ids assigned to each role. It is not a compiled skill; {@link #compile()} turns
 * it into an executable {@link SkillEffect} on demand (at cast time) via
 * {@link SkillCompiler}, so storage stays small and data-driven.
 *
 * The dual cost model means fuel aspects are gone: aspects are a gate, mana is the
 * consumable, so a recipe is just carrier + effects + modifiers.
 */
public record StoredSkill(String name,
    ...
    /** Resolve the aspect ids and compile to an executable skill. */
